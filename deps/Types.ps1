...
function ConvertTo-PmcProjectObject {
    <#
    .SYNOPSIS
    Converts a string project name or ensures project object has proper structure

    .DESCRIPTION
    Handles the common pattern of projects being stored as either strings or objects.
    Converts string to object with basic properties if needed.

    .PARAMETER Project
    The project (string or object) to normalize

    .PARAMETER UpdateInArray
    If provided, updates the project in the data array at the matching index

    .OUTPUTS
    PSCustomObject with project properties
    #>
    param(
        $Project,
        [ref]$DataArray,
    ...
    if ($Project -is [string]) {
        ...
        # Update in array if reference provided
        if ($DataArray -and $Index -ge 0 -and $Index -lt $DataArray.Value.Count) {
            $DataArray.Value[$Index] = $projectObj
        }
    }

    # Ensure all required properties exist
    Ensure-PmcProjectProperties $projectObj

    return $projectObj
}

Export-ModuleMember -Function ConvertTo-PmcTokens, Pmc-HasProp, Pmc-GetProp, Ensure-PmcTaskProperties, Ensure-PmcProjectProperties, ConvertTo-PmcProjectObject
