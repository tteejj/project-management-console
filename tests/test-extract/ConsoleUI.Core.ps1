...
class PmcConsoleUIApp {
    ...
    [void] DrawTaskEditForm() {
        ...
        $this.terminal.WriteAtColor(4, 6, "Editing multiple fields. Tab/Shift+Tab navigate. Enter saves. Esc cancels.", [PmcVT100]::Yellow(), "")

        $this.terminal.FillArea(0, $this.terminal.Height - 1, $this.terminal.Width, 1, ' ')
        $this.terminal.WriteAt(2, $this.terminal.Height - 1, "Tab/Shift+Tab navigate  |  Enter: Save  |  Esc: Cancel")
    }
    ...
    [void] HandleTaskEditForm() {
        ...
        try {
            ...
            $this.DrawTaskEditForm()

            # Multi-field editor with Tab/Shift+Tab navigation
            $yStart = 8
            $pri = try { if ($task.PSObject.Properties['priority']) { [string]$task.priority } else { '' } } catch { '' }
            $dueVal = ''
            if ($task.PSObject.Properties['due'] -and $task.due) { $dueVal = [string]$task.due }
            elseif ($task.PSObject.Properties['dueDate'] -and $task.dueDate) { $dueVal = [string]$task.dueDate }
            $statusVal = try { if ($task.PSObject.Properties['status']) { [string]$task.status } else { '' } } catch { '' }

            $fields = @(
                @{ Name='text';     Label='Text:';      Value=([string]$task.text);   X=12; Y=$yStart }
                @{ Name='project';  Label='Project:';   Value=([string]$task.project);X=12; Y=($yStart+3) }
                @{ Name='priority'; Label='Priority:';  Value=$pri;                   X=12; Y=($yStart+6) }
                @{ Name='due';      Label='Due (YYYY-MM-DD or blank):'; Value=$dueVal; X=32; Y=($yStart+9) }
                @{ Name='status';   Label='Status (pending/active/completed):'; Value=$statusVal; X=38; Y=($yStart+12) }
            )

            # Draw labels and current values
            foreach ($f in $fields) {
                $this.terminal.WriteAtColor(4, $f['Y'], $f['Label'], [PmcVT100]::Yellow(), "")
                $this.terminal.WriteAt($f['X'], $f['Y'], [string]$f['Value'])
            }
            ...
            while ($true) {
                ...
                if ($k.Key -eq 'Enter') {
                    ...
                    $isShift = ("" + $k.Modifiers) -match 'Shift'
                    if ($isShift) { $active = ($active - 1); if ($active -lt 0) { $active = $fields.Count - 1 } }
                    else { $active = ($active + 1) % $fields.Count }
                    continue
                } elseif ($k.Key -eq 'Backspace') {
                    if ($buf.Length -gt 0) {
                        $buf = $buf.Substring(0, $buf.Length - 1)
                        $fields[$active]['Value'] = $buf
                        $this.terminal.FillArea($col, $row, $this.terminal.Width - $col - 1, 1, ' ')
                        $this.terminal.WriteAt($col, $row, $buf)
                    }
                    continue
                } else {
            ...
            $origText     = ($fields | Where-Object { $_['Name'] -eq 'text' }     | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })
            $origProject  = ($fields | Where-Object { $_['Name'] -eq 'project' }  | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })
            $origPriority = ($fields | Where-Object { $_['Name'] -eq 'priority' } | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })
            $origDue      = ($fields | Where-Object { $_['Name'] -eq 'due' }      | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })
            $origStatus   = ($fields | Where-Object { $_['Name'] -eq 'status' }   | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })

            # Extract new values
            $newText     = ($fields | Where-Object { $_['Name'] -eq 'text' }     | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })
            $newProject  = ($fields | Where-Object { $_['Name'] -eq 'project' }  | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })
            $newPriority = ($fields | Where-Object { $_['Name'] -eq 'priority' } | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })
            $newDue      = ($fields | Where-Object { $_['Name'] -eq 'due' }      | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })
            $newStatus   = ($fields | Where-Object { $_['Name'] -eq 'status' }   | Select-Object -First 1 | ForEach-Object { [string]$_['Value'] })

            # Apply changes (only if different)
            $changed = $false
            if ($newText -ne $origText) { $task.text = $newText; $changed = $true }
            if ($newProject -ne $origProject) { $task.project = if ([string]::IsNullOrWhiteSpace($newProject)) { $null } else { $newProject }; $changed = $true }
            if ($newPriority -ne $origPriority) { $task.priority = ($newPriority ?? '').ToLower(); $changed = $true }

            # Due validation
            if ($newDue -ne $origDue) {
                if ([string]::IsNullOrWhiteSpace($newDue)) {
                    $task | Add-Member -MemberType NoteProperty -Name 'due' -Value $null -Force
                    if ($task.PSObject.Properties['dueDate']) { $task.dueDate = $null }
                    $changed = $true
                } else {
                    $isValid = $false
                    try {
                        if ($newDue -match '^\d{4}-\d{2}-\d{2}$') {
                            [void][datetime]::ParseExact($newDue,'yyyy-MM-dd',$null)
                            $isValid = $true
                        }
                    } catch { $isValid = $false }
                    if (-not $isValid) {
                        $this.terminal.WriteAtColor(4, [Math]::Max(6, $this.terminal.Height - 3), "Invalid due date. Use YYYY-MM-DD.", [PmcVT100]::Red(), "")
                        Start-Sleep -Seconds 2
                        $this.GoBackOr('tasklist'); return
                    }
                    $task | Add-Member -MemberType NoteProperty -Name 'due' -Value $newDue -Force
                    if ($task.PSObject.Properties['dueDate']) { $task.dueDate = $newDue }
                    $changed = $true
                }
            }

            if ($newStatus -ne $origStatus) { $task.status = ($newStatus ?? '').ToLower(); $changed = $true }
            ...
            if ($changed) {
                Save-PmcData -Data $data -Action "Edited task $taskId (multi)"
            }
            # Always refresh task list when exiting edit (prefer in-memory data to reflect even if WhatIf)
            $this.LoadTasks($data)

            # Return to previous view if set; otherwise task list. Keep selection if possible
            if ($this.previousView -and $this.previousView -ne 'taskdetail') { $this.currentView = $this.previousView; $this.previousView = '' }
            else { $this.currentView = 'tasklist' }
            # Try to keep selection on the edited task
            for ($i=0; $i -lt $this.tasks.Count; $i++) { if ($this.tasks[$i].id -eq $taskId) { $this.selectedTaskIndex = $i; break } }
            # Redraw immediately with updated selection
            $this.RefreshCurrentView()
        } catch {
            Show-InfoMessage -Message "Failed to edit task: $_" -Title "SAVE ERROR" -Color "Red"
            $this.GoBackOr('tasklist')
        }
    }
    ...
    [void] HandleEditProjectForm() {
        ...
        try {
            ...
            if (-not $project) { $this.terminal.WriteAtColor(4, 12, "Project '$projectName' not found", [PmcVT100]::Red(), ""); Start-Sleep -Seconds 2; $this.GoBackOr('projectlist'); return }
            ...
            $this.terminal.WriteAtColor(4, 12, "Error: $_", [PmcVT100]::Red(), ""); Start-Sleep -Seconds 2; $this.GoBackOr('projectlist')
        }
    }
