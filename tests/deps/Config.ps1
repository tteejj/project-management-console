...
function Save-PmcConfig {
    ...
    # Default: write to pmc/module/config.json (standalone root)
    try {
        try { $root = Get-PmcRootPath } catch { $root = (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) }
        $path = Join-Path $root 'config.json'
