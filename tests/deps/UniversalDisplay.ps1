...
# Provide a no-op shim to avoid errors when theme/config code calls this in standalone mode
if (-not (Get-Command Ensure-PmcUniversalDisplay -ErrorAction SilentlyContinue)) {
function Ensure-PmcUniversalDisplay { param(); return }
}
...
function Show-PmcOverdueTasksInteractive {
    ...
    Show-PmcData -DataType "task" -Filters @{
        "status" = "pending"
        "due_range" = "overdue"
    } -Title "OVERDUE TASKS" -Interactive -Context $Context
}
...
function Show-PmcHelpCategory {
    ...
    if (-not $Script:PmcHelpContent.ContainsKey($Category)) {
        Write-Host "WARNING: Category '$Category' not found in help content" -ForegroundColor Yellow
        return
    }
    ...
    $title = "HELP {0} - {1}" -f $Category, $categoryData.Description
