...
function Initialize-PmcSecuritySystem {
    ...
    if ($securityState.AllowedWritePaths.Count -eq 0) {
        try {
            ...
            # Fallback for standalone: try to find root from script location or current directory
            $root = $null
            if (-not [string]::IsNullOrEmpty($PSScriptRoot)) {
                # If in deps/ folder, go up one level
                if ($PSScriptRoot -like '*deps') {
                    $root = Split-Path $PSScriptRoot -Parent
                } else {
                    $root = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
                }
            }
            # Final fallback: use current directory if we have config.json or tasks.json there
    ...
    try {
        ...
        try {
            if ([System.IO.Path]::IsPathRooted($Path)) {
                ...
                try { $root = Get-PmcRootPath } catch { $root = (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) }
