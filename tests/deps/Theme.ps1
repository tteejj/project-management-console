...
function Set-PmcTheme {
    ...
    switch -Regex ($color.ToLower()) {
        ...
        '^matrix$'  { $hex = '#00ff66'; break }
        '^amber$'   { $hex = '#ffbf00'; break }
        '^synthwave$' { $hex = '#ff2bd6'; break }
        default     { }
    }
    ...
    try {
        ...
        Write-PmcStyled -Style 'Error' -Text ("Failed to save theme: {0}" -f $_)
    }
}
...
## Centralized: Set-PmcIconMode lives in deps/Config.ps1
...
function Edit-PmcTheme {
    ...
    while (-not $done) {
        # Render UI
        Write-Host ([PraxisVT]::Clear())
        Show-PmcHeader -Title 'THEME ADJUSTER'
        Write-Host ''
...
function Get-PmcThemeList { [PmcCommandContext]$Context | Out-Null; @('default','#33aaff','ocean','lime','purple','slate','matrix','amber','synthwave','#ff2bd6','high-contrast') | ForEach-Object { Write-Host $_ } }
...
## Centralized: Reload-PmcConfig lives in deps/Config.ps1

Export-ModuleMember -Function Initialize-PmcThemeSystem, Set-PmcTheme, Reset-PmcTheme, Edit-PmcTheme, Show-Slider, Show-PmcPreferences, Get-PmcThemeList, Apply-PmcTheme, Show-PmcThemeInfo
