...
export interface MainEngineConfig {
  maxThrustN?: number;              // Maximum thrust at full throttle
  specificImpulseSec?: number;      // Isp in seconds
  maxGimbalDeg?: number;            // Maximum gimbal angle
  ignitionTimeS?: number;           // Time to reach full thrust
  shutdownTimeS?: number;           // Time to spool down
  minThrottle?: number;             // Minimum throttle (e.g., 0.4 = 40%)
  chamberPressureBar?: number;      // Design chamber pressure
  chamberTempK?: number;            // Design chamber temperature
  nozzleAreaM2?: number;            // Nozzle exit area
  fuelOxidizerRatio?: number;       // Mass ratio (e.g., 2.5:1)
}

export class MainEngine {
  // Engine parameters
  public maxThrustN: number;
  public specificImpulseSec: number;
  public maxGimbalDeg: number;
  public ignitionTimeS: number;
  public shutdownTimeS: number;
  public minThrottle: number;
  public chamberPressureBar: number;
  public chamberTempK: number;
  public nozzleAreaM2: number;
  public fuelOxidizerRatio: number;

  // State
  public status: 'off' | 'igniting' | 'running' | 'shutdown';
  public throttle: number;              // 0.0 to 1.0
  public currentThrustN: number;        // Actual thrust output
  public gimbalPitchDeg: number;        // -max to +max
  public gimbalYawDeg: number;          // -max to +max

  // Internal state
  private ignitionProgress: number = 0;  // 0 to 1
  private shutdownProgress: number = 0;  // 0 to 1
  public currentChamberPressureBar: number = 0;
  public currentChamberTempK: number = 293;

  // Health and wear
  public health: number = 100;          // 0-100%
  public totalFiredSeconds: number = 0;
  public ignitionCount: number = 0;
  public restartCooldownS: number = 0;  // Minimum time between restarts

  // Consumption tracking
  public totalFuelConsumedKg: number = 0;
  public totalOxidizerConsumedKg: number = 0;

  // Events
  public events: Array<{ time: number; type: string; data: any }> = [];

  // Constants
  private readonly G0 = 9.80665; // Standard gravity (m/s²)
   ...
   */
  private updateEngineState(dt: number, time: number): void {
    switch (this.status) {
      case 'igniting':
   ...
   * where v_e = Isp * g0
   */
  private calculateThrust(dt: number): void {
    if (this.status === 'off') {
      this.currentThrustN = 0;
      return;
    }
   ...
   */
  private degradeHealth(dt: number): void {
    if (this.status === 'running' && this.currentThrustN > 0) {
   ...
   * ṁ = F / (Isp * g0)
   */
  public getMassFlowRateKgPerSec(): number {
    if (this.currentThrustN === 0) return 0;
   ...
   */
  public getConsumptionRates(): { fuelKgPerSec: number; oxidizerKgPerSec: number } {
   ...
   */
  public consumePropellant(dt: number, availableFuelKg: number, availableOxidizerKg: number):
    { fuelConsumed: number; oxidizerConsumed: number } {
  ...
  ignite(): boolean {
    ...
    if (this.restartCooldownS > 0) return false;
    if (this.health < 10) return false;
   ...
   */
  shutdown(): void {
    if (this.status === 'off' || this.status === 'shutdown') return;
   ...
   */
  getThrustVector(): { x: number; y: number; z: number } {
