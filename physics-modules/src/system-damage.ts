...
export interface ShipSystem {
  ...
  // Requirements
  powerDraw: number;            // kW
  requiresAtmosphere?: boolean; // Fails in vacuum
  dependencies?: string[];      // System IDs this depends on
  isCritical?: boolean;         // Critical for ship operation
}
 ...
 */
export class SystemDamageManager {
  private systems: Map<string, ShipSystem> = new Map();
  private hull: HullStructure;

  // Thresholds
  private readonly DEGRADED_THRESHOLD = 0.7;    // Below 70% = degraded
  private readonly OFFLINE_THRESHOLD = 0.3;     // Below 30% = offline
  private readonly DESTROYED_THRESHOLD = 0.1;   // Below 10% = destroyed
   ...
   */
  private updateSystemStatus(system: ShipSystem): void {
   ...
   */
  private applyCompartmentDamage(system: ShipSystem, dt: number): void {
   ...
   */
  private checkDependencies(system: ShipSystem): void {
    if (!system.dependencies || system.dependencies.length === 0) return;
  ...
  getDamageReport(): DamageReport {
    const damagedSystems: SystemFailure[] = [];
    const criticalFailures: SystemFailure[] = [];
   ...
   */
  private getFailureReason(system: ShipSystem): string {
    if (system.integrity <= this.DESTROYED_THRESHOLD) {
      return 'Destroyed';
    }
  ...
  getSystemEffectiveness(systemId: string): number {
    ...
    if (!system || !system.operational) return 0;
