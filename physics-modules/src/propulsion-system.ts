...
export interface ThrusterConfig {
  id: string;
  type: ThrusterType;
  position: Vector3;          // Location on ship (m, for torque calculation)
  ...
  efficiency: number;         // 0-1 (thrust efficiency → rest becomes heat)
}

export interface ThrusterState {
  enabled: boolean;
  throttle: number;           // 0-1
  gimbalAngle: Vector3;       // Current gimbal (degrees, not radians)
  ...
  actualThrust: number;       // N (current, after all modifiers)
}
...
export class PropulsionSystem {
  private thrusters: Map<string, { config: ThrusterConfig, state: ThrusterState }> = new Map();
  private fuelTanks: Map<string, FuelTankInterface> = new Map();
  private powerBudget?: PowerBudgetInterface;

  private readonly G0 = 9.80665;  // Standard gravity (m/s²)
  private readonly GIMBAL_RATE = 10;  // degrees per second
  ...
  fireThruster(id: string, throttle: number): boolean {
    ...
    // Check damage
    if (state.damaged || state.integrity <= 0) return false;
  ...
  setGimbal(id: string, targetAngle: Vector3): boolean {
    const thruster = this.thrusters.get(id);
    if (!thruster || !thruster.config.canGimbal) return false;
  ...
  getThrustVector(id: string): { force: Vector3, torque: Vector3 } {
    const thruster = this.thrusters.get(id);
    if (!thruster) {
      return { force: VectorMath.zero(), torque: VectorMath.zero() };
    }
    ...
    const force = VectorMath.scale(thrustDir, state.actualThrust);

    // Calculate torque: τ = r × F
    const torque = VectorMath.cross(config.position, force);
  ...
  update(dt: number): ThrustOutput {
    ...
    let totalTorque = VectorMath.zero();
    ...
    for (const [id, thruster] of this.thrusters.entries()) {
      ...
      if (thrustResult.success) {
        ...
        totalForce = VectorMath.add(totalForce, force);
        totalTorque = VectorMath.add(totalTorque, torque);
  ...
  private calculateThrust(
    thruster: { config: ThrusterConfig, state: ThrusterState },
    dt: number
  ): {
    success: boolean,
    thrust: number,
   ...
   * Calculate thrust direction with gimbal
   */
  private calculateThrustDirection(baseDirection: Vector3, gimbalAngle: Vector3): Vector3 {
    ...
    const thrustDir: Vector3 = {
      x: baseDirection.x + gimbalRad.y,
      y: baseDirection.y + gimbalRad.x,
      z: baseDirection.z
    };

    return VectorMath.normalize(thrustDir);
  }
   ...
   */
  getTotalThrust(): { force: Vector3, torque: Vector3 } {
    let totalForce = VectorMath.zero();
    let totalTorque = VectorMath.zero();
