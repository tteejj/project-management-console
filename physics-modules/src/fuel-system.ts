...
export class FuelSystem {
  ...
  public totalFuelConsumed: number = 0;
  ...
  private createDefaultTanks(): FuelTank[] {
    return [
      {
        id: 'main_1',
        volume: 100,
   ...
   */
  private updateTankPressures(): void {
   ...
   */
  private handleFuelTransfers(dt: number): void {
    for (const sourceTank of this.tanks) {
      if (!sourceTank.valves.crossfeedTo) continue;
      ...
      if (pressureDiff <= 0) continue; // No flow if no pressure difference
   ...
   */
  private handleVenting(dt: number): void {
    for (const tank of this.tanks) {
      if (tank.valves.vent) {
   ...
   */
  getFuelBalance(): { offset: Vector2; magnitude: number } {
    let totalMass = 0;
    let weightedX = 0;
    let weightedY = 0;
  ...
  setCrossfeed(sourceTankId: string, destTankId: string | undefined): boolean {
    ...
    if (!sourceTank) return false;

    if (destTankId) {
      const destTank = this.getTank(destTankId);
      if (!destTank) return false;
