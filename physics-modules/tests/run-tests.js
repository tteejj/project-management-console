...
try {
  ...
  console.log('\n' + '='.repeat(60) + '\n');
  console.log('Running life support system tests...\n');

  // Compile life support test file
  execSync('npx tsc tests/life-support-system.test.ts --outDir dist --module commonjs --target es2020 --esModuleInterop --skipLibCheck', {
    cwd: path.join(__dirname, '..'),
    stdio: 'inherit'
  });

  // Run test
  execSync('node dist/tests/life-support-system.test.js', {
    cwd: path.join(__dirname, '..'),
    stdio: 'inherit'
  });
