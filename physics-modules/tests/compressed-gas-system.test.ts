...
import { CompressedGasSystem } from '../src/compressed-gas-system';
...
class CompressedGasSystemTester {
  ...
  private testIdealGasLaw(): void {
    ...
    this.assert(
      newTemp > initialTemp,
      'Temperature increases with ambient',
      'Should heat up'
    );
    ...
    this.assertClose(
      newPressure,
      expectedPressure,
      20.0, // Allow tolerance for equilibration dynamics
    ...
    this.assert(
      coolPressure < heatedPressure,
      'Pressure decreases when cooled',
      'Should follow ideal gas law'
    );
  }
   ...
   */
  private testGasConsumption(): void {
    ...
    this.assertClose(
      ...
      initialMass - 2.0,
      0.01,
      'Mass reduced correctly'
    );

    this.assert(
      newPressure < initialPressure,
      'Pressure drops after consumption',
      'Less gas = lower pressure'
    );
    ...
    this.assert(
      consumedExcess <= availableMass,
      'Cannot consume more than available',
      `Should be limited to ${availableMass}kg, got ${consumedExcess}kg`
    );
  ...
  private testPressureDrop(): void {
    ...
    this.assert(
      readings[0].pressure > readings[1].pressure,
      'Pressure drops after first consumption',
      'Should decrease'
    );

    this.assert(
      readings[1].pressure > readings[2].pressure,
    ...
    this.assertClose(
      ratio1,
      massRatio1,
      0.07, // Slightly higher tolerance due to rounding
   ...
   */
  private testRegulatorOperation(): void {
    ...
    this.assert(
      flowRate > 0,
      'Regulator provides flow when activated',
      'Should have positive flow rate'
    );
    ...
    this.assert(
      !regAfter!.active,
      'Regulator deactivates correctly',
      'Should be inactive'
    );
   ...
   */
  private testRegulatorPressureControl(): void {
    ...
    this.assertClose(
      ...
      2.5,
      0.1,
      'Regulator maintains target pressure'
    );
    ...
    this.assert(
      lowPressureOutput < 2.5 || bottle.pressureBar < 7.5,
      'Regulator output drops when bottle very depleted',
      'Cannot maintain pressure without sufficient input'
    );
   ...
   */
  private testOverpressureWarning(): void {
    ...
    this.assert(
      warning !== undefined,
      'Overpressure warning generated',
      'Should warn near max pressure'
    );
   ...
   */
  private testBottleRupture(): void {
    ...
    this.assert(
      bottle.ruptured,
      'Bottle ruptures when overpressure',
      'Should rupture above max pressure'
    );
  ...
  private testGasTransfer(): void {
    ...
    this.assertClose(
      ...
      5.0,
      0.01,
      'Destination bottle gains mass'
    );

    this.assert(
      bottle2.pressureBar > 50,
      'Destination pressure increases',
  ...
  private testTemperatureEquilibration(): void {
    ...
    this.assertClose(
      finalTemp,
      350,
      15.0, // Within 15K (thermal equilibration is slow by design)
  ...
  private testWarningGeneration(): void {
    ...
    const system1 = new CompressedGasSystem({
      bottles: [{
        gas: 'O2',
    ...
    this.assert(
      lowPressureEvent !== undefined,
      'Low pressure warning generated',
      'Should warn when pressure < 30 bar'
    );

    console.log(`  Low pressure warning: ✓`);

    // Test depletion warning
    const system2 = new CompressedGasSystem({
      bottles: [{
        gas: 'He',
        pressureBar: 5,
        volumeL: 30,
    ...
    this.assert(
      depletedEvent !== undefined,
      'Depletion warning generated',
      'Should warn when nearly empty'
    );
