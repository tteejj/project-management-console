...
import { FuelSystem } from '../src/fuel-system';
...
class FuelSystemTester {
   ...
   */
  private testPressureDynamics(): void {
    ...
    this.assert(
      ...
      'Pressure decreases as fuel consumed',
      `Pressure should decrease, but went from ${initialPressure} to ${newPressure}`
    );
    ...
    this.assert(
      ...
      `Pressure should keep decreasing`
    );
  }
  ...
  private testCenterOfMass(): void {
    ...
    this.assert(
      unbalancedBalance.magnitude > initialBalance.magnitude,
      'Imbalance increases COM offset',
      'Magnitude should increase when tanks are unbalanced'
    );

    this.assert(
      unbalancedBalance.offset.y > initialBalance.offset.y,
      'COM shifts toward fuller tank',
      'Should shift toward tank 2'
    );
    ...
    this.assertClose(
      emptyBalance.magnitude,
      0,
      0.01,
      'Empty tanks have zero COM offset'
    );
  }
  ...
  private testCrossfeed(): void {
    ...
    this.assert(
      tank1.fuelMass < beforeTransfer1,
      'Source tank loses fuel during crossfeed',
      'Tank1 should have less fuel'
    );

    this.assert(
      tank2.fuelMass > beforeTransfer2,
      'Destination tank gains fuel during crossfeed',
      'Tank2 should have more fuel'
    );
  ...
  private testVenting(): void {
    ...
    this.assert(
      tank.fuelMass < initialFuel,
      'Venting reduces fuel mass',
      'Fuel should be vented'
    );
  ...
  private testWarnings(): void {
    ...
    this.assert(
      lowFuelWarning !== undefined,
      'Low fuel warning generated',
      'Should generate low fuel warning'
    );
    ...
    this.assert(
      ...
      'Tank empty warning generated',
      'Should generate empty tank warning'
    );
  ...
  private testFuelLines(): void {
    ...
    this.assertClose(
      state.fuelLines.mainEngine.pressure,
      tank.pressureBar,
      0.01,
      'Line pressure equals tank pressure without pump'
    );
    ...
    this.assert(
      pumpedState.fuelLines.mainEngine.pressure > tank.pressureBar,
      'Pump increases line pressure',
      'Fuel pump should boost pressure'
    );

    this.assertClose(
      pumpedState.fuelLines.mainEngine.pressure,
      tank.pressureBar + 5,
      0.01,
      'Pump adds 5 bar'
    );
