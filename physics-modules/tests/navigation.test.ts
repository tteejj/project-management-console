...
class NavigationTests {
  ...
  testTrajectoryFreefallImpact(): void {
    console.log('\nTest 1: Trajectory Predictor - Freefall Impact');
    const predictor = new TrajectoryPredictor();

    const position: Vector3 = { x: 0, y: 0, z: this.MOON_RADIUS + 1000 };  // 1km altitude
    const velocity: Vector3 = { x: 0, y: 0, z: -50 };  // 50 m/s downward
    const thrust = 0;  // No thrust
    const thrustDir: Vector3 = { x: 0, y: 0, z: 1 };
  ...
  testTrajectoryWithThrust(): void {
    console.log('\nTest 2: Trajectory Predictor - With Thrust');
    const predictor = new TrajectoryPredictor();

    const position: Vector3 = { x: 0, y: 0, z: this.MOON_RADIUS + 500 };
    const velocity: Vector3 = { x: 0, y: 0, z: -40 };
    const thrust = 5000;  // 5kN thrust (slight deceleration)
    const thrustDir: Vector3 = { x: 0, y: 0, z: 1 };  // Upward
  ...
  testTrajectoryEscaping(): void {
    console.log('\nTest 3: Trajectory Predictor - Escaping');
    const predictor = new TrajectoryPredictor();

    const position: Vector3 = { x: 0, y: 0, z: this.MOON_RADIUS + 5000 };
    const velocity: Vector3 = { x: 0, y: 0, z: 2000 };  // Very high upward velocity
    const thrust = 0;
    const thrustDir: Vector3 = { x: 0, y: 0, z: 1 };
  ...
  testTrajectoryCoordinates(): void {
    console.log('\nTest 4: Trajectory Predictor - Coordinates');
    const predictor = new TrajectoryPredictor();

    const position: Vector3 = { x: 0, y: 0, z: this.MOON_RADIUS + 500 };
    const velocity: Vector3 = { x: 0, y: 0, z: -50 };
    const thrust = 0;
    const thrustDir: Vector3 = { x: 0, y: 0, z: 1 };
  ...
  testSuicideBurnBasic(): void {
    console.log('\nTest 5: Suicide Burn - Basic Calculation');
    const calculator = new SuicideBurnCalculator();
  ...
  testSuicideBurnShouldBurn(): void {
    console.log('\nTest 6: Suicide Burn - Should Burn Now');
    const calculator = new SuicideBurnCalculator();
  ...
  testSuicideBurnInsufficientThrust(): void {
    console.log('\nTest 7: Suicide Burn - Insufficient Thrust');
    const calculator = new SuicideBurnCalculator();
  ...
  testVelocityDecomposerVertical(): void {
    console.log('\nTest 9: Velocity Decomposer - Vertical Component');
    const decomposer = new VelocityDecomposer();

    const position: Vector3 = { x: 0, y: 0, z: this.MOON_RADIUS };
    const velocity: Vector3 = { x: 0, y: 0, z: -50 };  // Pure radial
  ...
  testVelocityDecomposerHorizontal(): void {
    console.log('\nTest 10: Velocity Decomposer - Horizontal Component');
    const decomposer = new VelocityDecomposer();

    const position: Vector3 = { x: 0, y: 0, z: this.MOON_RADIUS };
    const velocity: Vector3 = { x: 100, y: 0, z: 0 };  // Pure tangential
  ...
  testVelocityDecomposerMixed(): void {
    console.log('\nTest 11: Velocity Decomposer - Mixed Velocity');
    const decomposer = new VelocityDecomposer();

    const position: Vector3 = { x: 0, y: 0, z: this.MOON_RADIUS };
    const velocity: Vector3 = { x: 30, y: 40, z: -50 };  // Mixed
  ...
  testNavballDisplay(): void {
    console.log('\nTest 12: Navball Display - Render');
    const navball = new NavballDisplay();
  ...
  testNavballDisplayWithTarget(): void {
    console.log('\nTest 13: Navball Display - With Target');
    const navball = new NavballDisplay();

    const attitude = createIdentityQuaternion();
    const velocity: Vector3 = { x: 0, y: 0, z: 50 };
    const target: Vector3 = { x: 1000, y: 0, z: 0 };
  ...
  testNavigationTelemetry(): void {
    ...
    const telemetry = nav.getTelemetry(
      position,
      velocity,
      attitude,
      5000,      // mass
  ...
  testNavigationImpactPrediction(): void {
    console.log('\nTest 15: Navigation System - Impact Prediction');
    const nav = new NavigationSystem();

    const position: Vector3 = { x: 0, y: 0, z: this.MOON_RADIUS + 1000 };
    const velocity: Vector3 = { x: 0, y: 0, z: -50 };
    const thrustDir: Vector3 = { x: 0, y: 0, z: 1 };
  ...
  testNavigationSuicideBurn(): void {
    console.log('\nTest 16: Navigation System - Suicide Burn Calculation');
    const nav = new NavigationSystem();
  ...
  testNavigationVelocityDecomp(): void {
    console.log('\nTest 17: Navigation System - Velocity Decomposition');
    const nav = new NavigationSystem();
  ...
  testNavigationTarget(): void {
    console.log('\nTest 18: Navigation System - Target Setting');
    const nav = new NavigationSystem();

    const target: Vector3 = { x: 10000, y: 0, z: this.MOON_RADIUS };
    ...
    const telemetry = nav.getTelemetry(
      position, velocity, attitude, 5000, 45000, thrustDir, 1.0, 200, 300, 311
    );
  ...
  testNavigationClearTarget(): void {
    console.log('\nTest 19: Navigation System - Clear Target');
    const nav = new NavigationSystem();
  ...
  testTelemetryDeltaV(): void {
    ...
    const telemetry = nav.getTelemetry(
      position, velocity, attitude, mass, 45000, thrustDir, 1.0, fuelMass, 300, Isp
    );
  ...
  testTelemetryTWR(): void {
    ...
    const telemetry = nav.getTelemetry(
      position, velocity, attitude, mass, thrust, thrustDir, 1.0, 200, 300, 311
    );
  ...
  testTelemetryAngleFromVertical(): void {
    console.log('\nTest 23: Telemetry - Angle From Vertical');
    const nav = new NavigationSystem();
    ...
    const thrustDir: Vector3 = { x: 0, y: 0, z: 1 };  // Thrust upward
  ...
  testVelocityDecomposerPrograde(): void {
    console.log('\nTest 24: Velocity Decomposer - Prograde Component');
    const decomposer = new VelocityDecomposer();

    const position: Vector3 = { x: 0, y: 0, z: this.MOON_RADIUS };
    const velocity: Vector3 = { x: 50, y: 50, z: -50 };
