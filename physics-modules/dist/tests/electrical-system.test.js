...
const electrical_system_1 = require("../src/electrical-system");
class ElectricalSystemTester {
    ...
    testReactorStartup() {
        ...
        const system = new electrical_system_1.ElectricalSystem();
        // Initially offline
        this.assert(system.reactor.status === 'offline', 'Reactor starts offline', 'Reactor should be offline initially');
        // Start reactor
        const started = system.startReactor();
        this.assert(started, 'Reactor start command succeeds', 'Should accept start command');
        this.assert(system.reactor.status === 'starting', 'Reactor enters starting state', 'Should transition to starting');
        console.log(`  Status after start command: ${system.reactor.status}`);
        // Simulate 10 seconds - should still be starting
        for (let i = 0; i < 10; i++) {
            system.update(1.0, i);
        }
        this.assert(system.reactor.status === 'starting', 'Reactor still starting after 10 seconds', 'Should not be online yet');
        console.log(`  Status after 10 seconds: ${system.reactor.status}`);
        // Simulate to 30 seconds - should be online
        for (let i = 10; i < 30; i++) {
            system.update(1.0, i);
        }
        this.assert(system.reactor.status === 'online', 'Reactor online after 30 seconds', 'Should complete startup');
        ...
        this.assert(onlineEvent !== undefined, 'Reactor online event generated', 'Should log online event');
    }
    /**
     * TEST 2: Reactor Power Generation
     * Verify power output at different throttle settings
     */
    testReactorPowerGeneration() {
        ...
        this.assertClose(state.reactor.outputKW, expectedPower, 0.01, 'Power output matches throttle setting (50%)');
        // Set throttle to 100%
        ...
        this.assertClose(fullState.reactor.outputKW, expectedFullPower, 0.01, 'Power output matches throttle setting (100%)');
        ...
        this.assertClose(fullState.reactor.heatGenerationW, expectedWasteHeat, 10, 'Waste heat generation calculated correctly');
    }
    /**
    ...
    testReactorSCRAM() {
        ...
        this.assert(system.reactor.status === 'scrammed', 'Reactor SCRAMs when overtemp', 'Should emergency shutdown');
        this.assertClose(system.reactor.currentOutputKW, 0, 0.01, 'Power output zero after SCRAM');
        this.assertClose(system.reactor.throttle, 0, 0.01, 'Throttle reset to zero after SCRAM');
        // Check SCRAM event
        const events = system.getEvents();
        const scramEvent = events.find(e => e.type === 'reactor_scram');
        this.assert(scramEvent !== undefined, 'SCRAM event generated', 'Should log SCRAM event');
        // Test reset (should fail - still hot)
        const resetSuccess = system.resetReactor();
        this.assert(!resetSuccess, 'Cannot reset reactor while hot', 'Should prevent reset when temp too high');
        // Cool down and test reset
        system.reactor.temperature = 700; // below maxSafeTemp
        const resetSuccess2 = system.resetReactor();
        this.assert(resetSuccess2, 'Can reset reactor when cooled', 'Should allow reset when safe');
        this.assert(system.reactor.status === 'offline', 'Reactor offline after reset', 'Should return to offline state');
        console.log('');
    }
    /**
    ...
    testBatteryCharge() {
        ...
        this.assert(finalCharge > initialCharge, 'Battery charges from surplus power', 'Charge should increase');
        // Battery should not exceed capacity
        this.assert(finalCharge <= system.battery.capacityKWh, 'Battery does not overcharge', 'Should respect capacity limit');
        console.log('');
    }
    /**
    ...
    testBatteryDischarge() {
        ...
        this.assert(finalCharge < initialCharge, 'Battery discharges to supply load', 'Charge should decrease');
        // Battery should not go negative
        this.assert(finalCharge >= 0, 'Battery does not go negative', 'Should stop at zero');
        console.log('');
    }
    /**
    ...
    testCapacitorOperation() {
        ...
        this.assert(afterDeficit < initialCapCharge, 'Capacitor discharges during deficit', 'Should supply power during deficit');
        ...
        this.assert(afterCharge > beforeCharge, 'Capacitor charges from surplus', 'Should charge during surplus');
        console.log('');
    }
    /**
    ...
    testCircuitBreakerOvercurrent() {
        ...
        this.assert(breaker.tripped, 'Breaker trips on overcurrent', 'Should trip when current exceeds threshold');
        this.assert(!breaker.on, 'Breaker opens when tripped', 'Should turn off when tripped');
        // Check for trip event
        const events = system.getEvents();
        const tripEvent = events.find(e => e.type === 'breaker_tripped');
        this.assert(tripEvent !== undefined, 'Breaker trip event generated', 'Should log trip event');
        console.log('');
    }
    /**
    ...
    testPowerBusLoads() {
        ...
        this.assertClose(busA.loadKW, expectedBusA, 0.01, 'Bus A load calculated correctly');
        this.assertClose(busB.loadKW, expectedBusB, 0.01, 'Bus B load calculated correctly');
        console.log('');
    }
    /**
    ...
    testCrosstie() {
        ...
        this.assertClose(busAAfter.loadKW, busBAfter.loadKW, 0.01, 'Cross-tie balances bus loads');
        ...
        this.assertClose(totalAfter, totalBefore, 0.01, 'Total load conserved with cross-tie');
        console.log('');
    }
    /**
    ...
    testBatteryThermal() {
        ...
        this.assert(tempAfterDischarge > initialTemp, 'Battery heats up during discharge', 'Inefficiency generates heat');
        ...
        this.assert(tempAfterCooling < tempAfterDischarge, 'Battery cools down passively', 'Should cool toward ambient');
        console.log('');
    }
    /**
    ...
    testBatteryHealthDegradation() {
        ...
        this.assert(system.battery.chargeCycles > 500, 'Charge cycles accumulate over time', 'Should track cycles');
        this.assert(system.battery.health < 100, 'Battery health degrades after 500 cycles', 'Health should decrease');
        console.log('');
    }
    /**
    ...
    testBlackout() {
        ...
        this.assert(nonEssentialOnAfter < nonEssentialOnBefore, 'Blackout trips non-essential breakers', 'Should shed non-essential loads');
        ...
        this.assert(o2Gen.on, 'Essential breakers remain on (O2 Gen)', 'Should protect essential systems');
        this.assert(co2Scrub.on, 'Essential breakers remain on (CO2 Scrubber)', 'Should protect essential systems');
        // Check for blackout event
        const events = system.getEvents();
        const blackoutEvent = events.find(e => e.type === 'blackout');
        this.assert(blackoutEvent !== undefined, 'Blackout event generated', 'Should log blackout event');
        console.log('');
    }
    /**
    ...
    testWarnings() {
        ...
        const system1 = new electrical_system_1.ElectricalSystem({
            battery: {
        ...
        this.assert(lowBattEvent !== undefined, 'Low battery warning generated', 'Should warn when battery < 20%');
        console.log(`  Low battery warning: ✓`);
        // Test reactor overtemp warning
        const system2 = new electrical_system_1.ElectricalSystem({
            reactor: {
        ...
        this.assert(overtempEvent !== undefined, 'Reactor overtemp warning generated', 'Should warn when temp > maxSafeTemp');
        console.log(`  Reactor overtemp warning: ✓`);
        // Test battery overtemp warning
        const system3 = new electrical_system_1.ElectricalSystem({
            battery: {
        ...
        this.assert(battOvertempEvent !== undefined, 'Battery overtemp warning generated', 'Should warn when battery too hot');
        console.log(`  Battery overtemp warning: ✓`);
        // Test bus overload warning
        const system4 = new electrical_system_1.ElectricalSystem();
        ...
        this.assert(overloadEvent !== undefined, 'Bus overload warning generated', 'Should warn when bus exceeds capacity');
