...
const ship_physics_1 = require("../src/ship-physics");
class ShipPhysicsTester {
    ...
    assertClose(actual, expected, tolerance, name) {
        ...
        this.assert(passed, name, `Expected ${expected.toExponential(3)}, got ${actual.toExponential(3)} (diff: ${diff.toExponential(3)}, tol: ${tolerance.toExponential(3)})`, { actual, expected, diff });
    }
    /**
     * TEST 1: Gravitational Acceleration
     * Verify g = -G*M/r² at Moon surface
     */
    testGravitationalAcceleration() {
        console.log('Test 1: Gravitational Acceleration (g = -G*M/r²)');
        const ship = new ship_physics_1.ShipPhysics({
        ...
        this.assertClose(Math.abs(accel), expectedG, 0.01, 'Surface gravity matches g = G*M/r²');
        console.log('');
    }
    /**
    ...
    testFreefall() {
        ...
        this.assert(finalAltitude < initialAltitude, 'Altitude decreases during freefall', 'Ship should fall');
        this.assert(verticalSpeed < -5, 'Falling with significant downward velocity', 'Should be moving down');
        ...
        this.assertClose(verticalSpeed, expectedSpeed, 2.0, 'Vertical speed matches v = gt');
        console.log('');
    }
    /**
    ...
    testThrustIntegration() {
        ...
        this.assertClose(finalVelocity.z, expectedVelocity, 3.0, 'Velocity matches thrust acceleration');
        console.log('');
    }
    /**
    ...
    testRotationalDynamics() {
        ...
        this.assertClose(angularVel.y, expectedAngularVel, 0.01, 'Angular velocity matches τ/I');
        console.log('');
    }
    /**
    ...
    testQuaternionNormalization() {
        ...
        this.assertClose(magnitude, 1.0, 0.001, 'Quaternion remains normalized');
        console.log('');
    }
    /**
    ...
    testMassTracking() {
        ...
        this.assertClose(consumed, 100, 0.1, 'Mass decreases by consumed propellant');
        this.assertClose(finalMass, 7900, 1.0, 'Final mass = dry + remaining propellant');
        console.log('');
    }
    /**
    ...
    testAltitudeCalculation() {
        ...
        this.assertClose(altitude, 5000, 1.0, 'Altitude calculated correctly');
        ...
        this.assertClose(offAxisAltitude, 7.2, 1.0, 'Off-axis altitude correct');
        console.log('');
    }
    /**
    ...
    testEulerAngleConversion() {
        ...
        this.assertClose(euler.roll, 0, 0.1, 'Identity quaternion gives zero roll');
        this.assertClose(euler.pitch, 0, 0.1, 'Identity quaternion gives zero pitch');
        ...
        this.assert(Math.abs(euler.pitch) > 5, 'Pitch angle changes after pitch torque', 'Should have noticeable pitch');
        console.log('');
    }
    /**
    ...
    testGroundImpact() {
        ...
        this.assert(impactEvent !== undefined, 'Ground impact event generated', 'Should detect impact');
        if (impactEvent) {
    ...
    testGyroscopicEffect() {
        ...
        this.assert(Math.abs(finalAngularVelX) > Math.abs(initialAngularVelX), 'Gyroscopic coupling affects rotation', 'Should see coupling between axes');
        console.log('');
    }
    printResults() {
