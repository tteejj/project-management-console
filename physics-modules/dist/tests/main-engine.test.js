...
const main_engine_1 = require("../src/main-engine");
class MainEngineTester {
     ...
     */
    testIgnitionSequence() {
        console.log('Test 1: Ignition Sequence and Startup Time');
        const engine = new main_engine_1.MainEngine();
        console.log(`  Initial status: ${engine.status}`);
        this.assert(engine.status === 'off', 'Engine starts offline', 'Should be off initially');
        // Ignite
        const igniteSuccess = engine.ignite();
        console.log(`  Ignition command: ${igniteSuccess}, status: ${engine.status}`);
        this.assert(igniteSuccess, 'Ignition succeeds', 'Should allow ignition');
        this.assert(engine.status === 'igniting', 'Engine enters igniting state', 'Should be igniting');
        // Set throttle to 100%
        ...
        this.assert(engine.status === 'igniting', 'Still igniting after 1 second', 'Should not be fully started yet');
        ...
        this.assert(engine.status === 'running', 'Engine running after ignition time', 'Should be fully started');
        this.assertClose(engine.currentThrustN, engine.maxThrustN, 1000, 'Thrust at maximum after startup');
        console.log('');
    }
    /**
    ...
    testThrustCalculation() {
        ...
        this.assertClose(thrust, calculatedThrust, 100, 'Thrust matches F = ṁ * v_e');
        console.log('');
    }
    /**
    ...
    testMassFlowRate() {
        ...
        this.assertClose(massFlow, expectedMassFlow, 0.01, 'Mass flow rate calculated correctly');
        console.log('');
    }
    /**
    ...
    testPropellantConsumption() {
        ...
        this.assertClose(actualRatio, engine.fuelOxidizerRatio, 0.01, 'Mixture ratio correct');
        ...
        this.assertClose(sumRates, totalFlow, 0.001, 'Consumption rates sum to total mass flow');
        ...
        this.assertClose(consumedRatio, engine.fuelOxidizerRatio, 0.05, 'Consumed propellant maintains mixture ratio');
        console.log('');
    }
    /**
    ...
    testThrottleControl() {
        ...
        for (const targetThrottle of throttles) {
            ...
            if (targetThrottle < engine.minThrottle && targetThrottle > 0) {
                this.assertClose(actualThrottle, engine.minThrottle, 0.01, `Throttle clamped to minimum (${(targetThrottle * 100).toFixed(0)}% -> ${(engine.minThrottle * 100).toFixed(0)}%)`);
            }
            else {
                this.assertClose(actualThrottle, targetThrottle, 0.01, `Throttle set correctly (${(targetThrottle * 100).toFixed(0)}%)`);
            }
        }
        console.log('');
    }
    ...
    testGimbalControl() {
        ...
        for (const test of testCases) {
            ...
            this.assertClose(engine.gimbalPitchDeg, test.expectPitch, 0.1, `Pitch gimbal ${test.pitch}° -> ${test.expectPitch}°`);
            this.assertClose(engine.gimbalYawDeg, test.expectYaw, 0.1, `Yaw gimbal ${test.yaw}° -> ${test.expectYaw}°`);
        }
        console.log('');
    }
    /**
    ...
    testShutdownSequence() {
        ...
        this.assert(engine.status === 'shutdown', 'Engine enters shutdown state', 'Should be shutting down');
        ...
        this.assert(engine.status === 'shutdown', 'Still shutting down', 'Should not be off yet');
        // Complete shutdown
        for (let i = 0; i < 5; i++) {
        ...
        this.assert(engine.status === 'off', 'Engine fully shutdown', 'Should be off');
        this.assert(engine.currentThrustN === 0, 'Zero thrust when off', 'Thrust should be zero');
        console.log('');
    }
    /**
    ...
    testHealthDegradation() {
        ...
        this.assert(finalHealth < initialHealth, 'Health degrades with use', 'Should decrease over time');
        this.assert(engine.totalFiredSeconds > 0, 'Fired time tracked', 'Should track total firing time');
        console.log('');
    }
    /**
    ...
    testPropellantStarvation() {
        ...
        this.assert(engine.status === 'shutdown' || engine.status === 'off', 'Engine shuts down on starvation', 'Should shutdown when starved');
        console.log('');
    }
    /**
    ...
    testRestartCooldown() {
        ...
        this.assert(engine.restartCooldownS > 0, 'Cooldown timer set', 'Should have cooldown after ignition');
        ...
        this.assert(!restartSuccess, 'Cannot restart during cooldown', 'Should prevent immediate restart');
        // Wait for cooldown
        for (let i = 0; i < 50; i++) {
        ...
        this.assert(delayedRestartSuccess, 'Can restart after cooldown', 'Should allow restart after cooldown');
        console.log('');
    }
    /**
    ...
    testThrustVector() {
        ...
        this.assertClose(vectorNoGimbal.x, 0, 100, 'No X component without gimbal');
        this.assertClose(vectorNoGimbal.y, 0, 100, 'No Y component without gimbal');
        this.assertClose(vectorNoGimbal.z, engine.currentThrustN, 100, 'Z component equals thrust');
        // With gimbal
        ...
        this.assert(Math.abs(vectorWithGimbal.x) > 100, 'X component present with pitch gimbal', 'Should have X component');
        this.assert(Math.abs(vectorWithGimbal.y) > 100, 'Y component present with yaw gimbal', 'Should have Y component');
        // Verify magnitude equals thrust
        const magnitude = Math.sqrt(vectorWithGimbal.x ** 2 +
        ...
        this.assertClose(magnitude, engine.currentThrustN, 100, 'Vector magnitude equals thrust');
        console.log('');
    }
    printResults() {
