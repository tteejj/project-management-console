...
class SpacecraftIntegrationTester {
    ...
    testSystemStartup() {
        ...
        this.assert(initialState.electrical.reactor.status === 'offline', 'Reactor starts offline', 'Should be offline initially');
        this.assert(initialState.mainEngine.status === 'off', 'Main engine starts off', 'Should be off initially');
        // Start reactor
        const reactorStarted = spacecraft.startReactor();
        console.log(`  Reactor start command: ${reactorStarted}`);
        this.assert(reactorStarted, 'Reactor start succeeds', 'Should allow reactor start');
        ...
        this.assert(stateAfterStartup.electrical.reactor.status === 'online', 'Reactor comes online after startup time', 'Should be online after 30s');
        // Start coolant pumps
        ...
        this.assert(coolantState.loops[0].pumpActive && coolantState.loops[1].pumpActive, 'Coolant pumps activated', 'Both pumps should be running');
        console.log('');
    }
    /**
    ...
    testPoweredDescent() {
        ...
        this.assert(engineIgnited, 'Main engine ignites successfully', 'Should allow ignition');
        // Set full throttle
        ...
        this.assert(finalState.physics.verticalSpeed > initialState.physics.verticalSpeed, 'Vertical speed decreases (slowing descent)', 'Engine should reduce descent rate');
        this.assert(finalState.fuel.totalFuel < initialState.fuel.totalFuel, 'Fuel mass decreases during burn', 'Should consume fuel');
        console.log('');
    }
    /**
    ...
    testAttitudeControl() {
        ...
        this.assert(Math.abs(finalAttitude.pitch - initialAttitude.pitch) > 5, 'RCS creates attitude change', 'Pitch should change significantly');
        // Deactivate
    ...
    testPowerManagement() {
        ...
        this.assert(powerState.reactor.status === 'online', 'Reactor online', 'Should be generating power');
        const netPower = powerState.reactor.outputKW - powerState.totalLoad;
        console.log(`  Net power: ${netPower.toFixed(1)} kW`);
        this.assert(netPower > 0 || powerState.battery.chargeKWh > 0, 'Sufficient power available', 'Should have power surplus or battery backup');
        console.log('');
    }
    /**
    ...
    testThermalIntegration() {
        ...
        const coolantState = spacecraft.getState().coolant;
        const reactorTemp = thermalState.components.find((c) => c.name === 'reactor')?.temperature || 0;
        const engineTemp = thermalState.components.find((c) => c.name === 'main_engine')?.temperature || 0;
        ...
        this.assert(engineTemp > 293, 'Engine heats up during burn', 'Should generate heat');
        this.assert(coolantState.totalHeatRejected > 0, 'Coolant rejects heat to space', 'Should radiate heat');
        console.log('');
    }
    /**
    ...
    testCompleteLandingSequence() {
        ...
        this.assert(landedSuccessfully || crashLanded, 'Landing sequence completes', 'Should reach surface');
