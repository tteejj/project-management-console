...
const thermal_system_1 = require("../src/thermal-system");
class ThermalSystemTester {
     ...
     */
    testComponentHeating() {
        console.log('Test 1: Component Temperature Rise from Heat Generation');
        const system = new thermal_system_1.ThermalSystem();
        ...
        this.assert(finalTemp > initialTemp, 'Component heats up with heat generation', 'Temperature should increase');
        this.assertClose(finalTemp, expectedTemp, 5.0, // Allow some tolerance for heat transfer to compartment
    ...
    testHeatTransferToCompartment() {
        ...
        this.assert(finalAirTemp > initialAirTemp, 'Compartment air heats up', 'Should absorb heat from hot component');
        this.assert(finalCompTemp < initialCompTemp + 150, // With cooling, shouldn't runaway
        'Component temperature stabilizes', 'Heat transfer prevents runaway heating');
        console.log('');
    }
    /**
    ...
    testCompartmentConduction() {
        ...
        this.assert(finalHot < initialHot, 'Hot compartment cools', 'Should lose heat');
        this.assert(finalCold > initialCold, 'Cold compartment warms', 'Should gain heat');
        this.assert((finalHot - finalCold) < (initialHot - initialCold), 'Temperature difference decreases', 'Compartments equilibrate');
        console.log('');
    }
    /**
    ...
    testOverheatingWarnings() {
        ...
        this.assert(overheatEvent !== undefined, 'Overheating warning generated', 'Should warn when component exceeds limit');
        if (overheatEvent) {
    ...
    testHeatAccumulation() {
        ...
        this.assertClose(totalHeat, expectedHeat, 100, 'Heat accumulation tracked correctly');
        console.log('');
    }
    /**
    ...
    testTemperatureEquilibration() {
        ...
        this.assert(Math.abs(tempChange2) < Math.abs(tempChange1), 'Temperature stabilizes over time', 'Rate of change should decrease');
        console.log('');
    }
    /**
    ...
    testMultipleHeatSources() {
        ...
        this.assert(finalTemp > initialTemp, 'Compartment heats from multiple sources', 'Should accumulate heat');
        this.assert(finalTemp > initialTemp + 5, 'Significant heating from multiple sources', 'Combined heat should be noticeable');
        console.log('');
    }
    printResults() {
