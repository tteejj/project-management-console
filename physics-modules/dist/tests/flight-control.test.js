...
const flight_control_1 = require("../src/flight-control");
// Test configuration
const defaultConfig = {
    pid: {
...
function createPitchQuaternion(angleDeg) {
...
class FlightControlTests {
    ...
    testPIDProportional() {
        console.log('\nTest 1: PID Controller - Proportional Response');
        const pid = new flight_control_1.PIDController({ kp: 1.0, ki: 0.0, kd: 0.0 });
        const error = 10;
    ...
    testPIDIntegral() {
        console.log('\nTest 2: PID Controller - Integral Accumulation');
        const pid = new flight_control_1.PIDController({ kp: 0.0, ki: 1.0, kd: 0.0 });
        let totalOutput = 0;
        for (let i = 0; i < 10; i++) {
    ...
    testPIDDerivative() {
        console.log('\nTest 3: PID Controller - Derivative Response');
        const pid = new flight_control_1.PIDController({ kp: 0.0, ki: 0.0, kd: 1.0 });
    ...
    testPIDAntiWindup() {
        console.log('\nTest 4: PID Controller - Anti-Windup');
        const pid = new flight_control_1.PIDController({ kp: 0.0, ki: 1.0, kd: 0.0, integralLimit: 10 });
    ...
    testPIDReset() {
        console.log('\nTest 5: PID Controller - Reset');
        const pid = new flight_control_1.PIDController({ kp: 1.0, ki: 1.0, kd: 1.0 });
    ...
    testSASStability() {
        console.log('\nTest 6: SAS - Stability Mode (Rate Damping)');
        const sas = new flight_control_1.SASController(defaultConfig);
    ...
    testAutopilotAltitudeHold() {
        console.log('\nTest 11: Autopilot - Altitude Hold');
        const autopilot = new flight_control_1.AutopilotSystem(defaultConfig);
    ...
    testAutopilotVerticalSpeedHold() {
        console.log('\nTest 12: Autopilot - Vertical Speed Hold');
        const autopilot = new flight_control_1.AutopilotSystem(defaultConfig);
    ...
    testAutopilotSuicideBurnCalculation() {
        console.log('\nTest 13: Autopilot - Suicide Burn Calculation');
        const autopilot = new flight_control_1.AutopilotSystem(defaultConfig);
    ...
    testAutopilotSuicideBurnActivation() {
        console.log('\nTest 14: Autopilot - Suicide Burn Activation');
        const autopilot = new flight_control_1.AutopilotSystem(defaultConfig);
        autopilot.setMode('suicide_burn');
    ...
    testGimbalAutopilot() {
        console.log('\nTest 18: Gimbal Autopilot - Horizontal Velocity Nulling');
        const gimbal = new flight_control_1.GimbalAutopilot();
    ...
    testFlightControlSystemIntegration() {
        console.log('\nTest 21: Flight Control System - Integration');
        const fcs = new flight_control_1.FlightControlSystem();
        fcs.setSASMode('stability');
        fcs.setAutopilotMode('altitude_hold');
        fcs.setTargetAltitude(1000);
        const state = fcs.update(createIdentityQuaternion(), { x: 0.1, y: 0, z: 0 }, null, { x: 0, y: 0, z: 50 }, { x: 0, y: 0, z: 1737400 + 500 }, 500, -10, 5000, 45000, 45000, 1.62, 0.1);
    ...
    testFlightControlSystemGetState() {
        console.log('\nTest 22: Flight Control System - Get State');
        const fcs = new flight_control_1.FlightControlSystem();
    ...
    testPIDAltitudeTuning() {
        console.log('\nTest 23: PID Tuning - Altitude Controller');
        const pid = new flight_control_1.PIDController(defaultConfig.pid.altitude);
        let altitude = 500;
        const target = 1000;
        const dt = 0.1;
    ...
    testSASControlAuthority() {
        console.log('\nTest 24: SAS - Control Authority Limiting');
        const config = { ...defaultConfig };
        config.sas.maxControlAuthority = 0.5; // Limit to 50%
        const sas = new flight_control_1.SASController(config);
        sas.setMode('attitude_hold');
    ...
    testPIDMultipleAxes() {
        console.log('\nTest 27: PID - Multiple Axes Independence');
        const pitchPID = new flight_control_1.PIDController(defaultConfig.pid.pitch);
        const rollPID = new flight_control_1.PIDController(defaultConfig.pid.roll);
    ...
    testSuicideBurnStateTracking() {
        console.log('\nTest 30: Flight Control - Suicide Burn State Tracking');
        const fcs = new flight_control_1.FlightControlSystem();
        fcs.setAutopilotMode('suicide_burn');
        // Above burn altitude
        let state = fcs.update(createIdentityQuaternion(), { x: 0, y: 0, z: 0 }, null, { x: 0, y: 0, z: -50 }, { x: 0, y: 0, z: 1737400 + 300 }, 300, -50, 5000, 45000, 0, 1.62, 0.1);
        ...
        // Below burn altitude
        state = fcs.update(createIdentityQuaternion(), { x: 0, y: 0, z: 0 }, null, { x: 0, y: 0, z: -50 }, { x: 0, y: 0, z: 1737400 + 100 }, 100, -50, 5000, 45000, 45000, 1.62, 0.1);
