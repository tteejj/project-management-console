...
const rcs_system_1 = require("../src/rcs-system");
class RCSSystemTester {
     ...
     */
    testIndividualThrusterControl() {
        console.log('Test 1: Individual Thruster Control');
        const rcs = new rcs_system_1.RCSSystem();
        ...
        this.assert(initialActive === 0, 'All thrusters start inactive', 'Should be 0 active thrusters');
        // Activate one thruster
        const success = rcs.activateThruster(0);
        console.log(`  Activate thruster 0: ${success}`);
        this.assert(success, 'Thruster activation succeeds', 'Should return true');
        const thruster0 = rcs.getThruster(0);
        this.assert(thruster0.active, 'Thruster 0 is active', 'Should be active');
        this.assert(rcs.getActiveThrusterCount() === 1, 'One thruster active', 'Count should be 1');
        // Deactivate
        rcs.deactivateThruster(0);
        console.log(`  Deactivated thruster 0, active: ${thruster0.active}`);
        this.assert(!thruster0.active, 'Thruster 0 is inactive', 'Should be inactive');
        console.log('');
    }
    /**
    ...
    testControlGroups() {
        ...
        this.assert(pitchUpGroup.active, 'Pitch up group activated', 'Should be active');
        ...
        this.assert(allActive, 'All thrusters in group are active', 'Should activate all thrusters');
        this.assert(rcs.getActiveThrusterCount() === pitchUpGroup.thrusterIds.length, 'Active count matches group size', 'Should match');
        // Deactivate group
        rcs.deactivateGroup('pitch_up');
        console.log(`  Deactivated pitch_up, active count: ${rcs.getActiveThrusterCount()}`);
        this.assert(rcs.getActiveThrusterCount() === 0, 'All thrusters deactivated', 'Should be 0');
        console.log('');
    }
    /**
    ...
    testThrustVectorCalculation() {
        ...
        const rcs = new rcs_system_1.RCSSystem({ thrusterThrustN: 25 });
        ...
        this.assertClose(thrust.x, 0, 1.0, 'No X thrust');
        this.assertClose(thrust.y, 50, // 2 thrusters * 25N
        1.0, 'Y thrust = 50N (2 thrusters)');
        this.assertClose(thrust.z, 0, 1.0, 'No Z thrust');
        ...
        this.assertClose(pitchThrust.z, 0, 1.0, 'Pitch down produces pure torque (zero net thrust)');
        const pitchTorque = rcs.getTotalTorque();
        console.log(`  Pitch down torque: τy=${pitchTorque.y.toFixed(1)} N·m`);
        this.assert(Math.abs(pitchTorque.y) > 100, 'Pitch down creates Y-axis torque', 'Should have significant torque');
        console.log('');
    }
    /**
     * TEST 4: Torque Calculation
     */
    testTorqueCalculation() {
        ...
        this.assert(Math.abs(pitchTorque.y) > 0.1, 'Pitch torque present', 'Should have Y-axis torque');
        rcs.deactivateGroup('pitch_up');
        // Roll CW
        ...
        this.assert(Math.abs(rollTorque.x) > 1.0, 'Roll torque present', 'Should have X-axis torque');
        console.log('');
    }
    /**
    ...
    testFuelConsumption() {
        ...
        this.assertClose(fuelConsumed, expectedFuel, 0.001, 'Fuel consumption matches calculation');
        console.log('');
    }
    /**
    ...
    testPulseCounting() {
        ...
        this.assert(thruster.pulseCount === 5, 'Pulse count incremented', 'Should count each activation');
        console.log('');
    }
    /**
     * TEST 7: Coupled Thruster Effects
     */
    testCoupledThrusters() {
        ...
        this.assert(active === 3, // 2 for pitch_down + 1 for yaw_left
        'Multiple groups active', 'Should have 3 thrusters');
        // Pitch creates pure torque (zero net thrust), yaw creates Y thrust
        this.assertClose(thrust.z, 0, 1.0, 'Pitch creates zero net thrust (pure torque)');
        this.assert(Math.abs(thrust.y) > 10, 'Y thrust from yaw', 'Should have yaw thrust');
        this.assert(Math.abs(torque.y) > 100, 'Pitch creates Y-axis torque', 'Should have pitch torque');
        console.log('');
    }
    printResults() {
