...
const coolant_system_1 = require("../src/coolant-system");
class CoolantSystemTester {
    ...
    testHeatAbsorption() {
        ...
        const system = new coolant_system_1.CoolantSystem();
        ...
        this.assert(finalTemp > initialTemp, 'Coolant absorbs heat from hot components', 'Temperature should increase');
        this.assert(finalTemp < 350, // Should be cooler than components due to radiation
        'Heat is balanced by radiation', 'Should not reach component temperature');
        console.log('');
    }
    /**
    ...
    testRadiationToSpace() {
        ...
        this.assert(finalTemp < initialTemp, 'Radiator cools via Stefan-Boltzmann radiation', 'Temperature should decrease');
        const state = system.getState();
        console.log(`  Total heat rejected: ${state.totalHeatRejected.toFixed(0)} J`);
        this.assert(state.totalHeatRejected > 0, 'Heat rejection tracked', 'Should track radiated energy');
        console.log('');
    }
    /**
     ...
     */
    testFlowRateDynamics() {
        console.log('Test 3: Flow Rate Dynamics');
        const system = new coolant_system_1.CoolantSystem();
        const loop = system.getLoop(0);
        console.log(`  Initial flow rate (pump off): ${loop.flowRateLPerMin} L/min`);
        this.assert(loop.flowRateLPerMin === 0, 'No flow when pump is off', 'Flow rate should be zero');
        // Start pump
        system.startPump(0);
        system.update(1.0, 0);
        console.log(`  Flow rate (pump on, full): ${loop.flowRateLPerMin.toFixed(1)} L/min`);
        this.assert(loop.flowRateLPerMin > 0, 'Flow starts when pump activated', 'Should have positive flow');
        this.assertClose(loop.flowRateLPerMin, loop.maxFlowRateLPerMin, 1.0, 'Flow rate at maximum when full');
        ...
        this.assertClose(loop.flowRateLPerMin, loop.maxFlowRateLPerMin * 0.5, 2.0, 'Flow rate proportional to coolant mass');
        // Stop pump
        system.stopPump(0);
        system.update(1.0, 2);
        console.log(`  Flow rate (pump stopped): ${loop.flowRateLPerMin} L/min`);
        this.assert(loop.flowRateLPerMin === 0, 'No flow when pump stopped', 'Flow rate should return to zero');
        console.log('');
    }
    /**
    ...
    testFreezingBehavior() {
        ...
        this.assert(loop.frozen, 'Coolant freezes below freezing point', 'Should be frozen');
        this.assert(!loop.pumpActive, 'Pump stops when frozen', 'Frozen pump should stop');
        const events = system.getEvents();
        const freezeEvent = events.find(e => e.type === 'coolant_frozen');
        this.assert(freezeEvent !== undefined, 'Freeze event generated', 'Should log freeze event');
        // Warm up
        loop.temperature = 270;
        system.update(1.0, 10);
        console.log(`  After warming to 270K: frozen=${loop.frozen}`);
        this.assert(!loop.frozen, 'Coolant thaws when warmed', 'Should not be frozen');
        console.log('');
    }
    /**
    ...
    testBoilingBehavior() {
        ...
        this.assert(loop.boiling, 'Coolant boils above boiling point', 'Should be boiling');
        const events = system.getEvents();
        const boilEvent = events.find(e => e.type === 'coolant_boiling');
        this.assert(boilEvent !== undefined, 'Boiling event generated', 'Should log boiling event');
        // Cool down
        loop.temperature = 370;
        system.update(1.0, 15);
        console.log(`  After cooling to 370K: boiling=${loop.boiling}`);
        this.assert(!loop.boiling, 'Coolant stops boiling when cooled', 'Should not be boiling');
        console.log('');
    }
    /**
    ...
    testCoolantLeaks() {
        ...
        this.assertClose(lossKg, expectedLoss, 0.1, 'Coolant loss matches leak rate');
        // Repair leak
        system.repairLeak(0);
        ...
        this.assertClose(loop.coolantMassKg, massBefore, 0.01, 'No more loss after repair');
        console.log('');
    }
    /**
    ...
    testCrossConnectBalancing() {
        ...
        this.assert(Math.abs(loop1.temperature - loop2.temperature) < 20, 'Temperatures equalize with cross-connect', 'Should reduce temperature difference');
        console.log('');
    }
    /**
    ...
    testPumpPowerConsumption() {
        ...
        this.assert(powerDraw === 0, 'No power when pumps off', 'Power should be zero');
        // Start both pumps
        system.startPump(0);
        system.startPump(1);
        ...
        this.assert(powerDraw === expectedPower, 'Power draw matches sum of active pumps', 'Should equal combined pump power');
        // Stop one pump
        system.stopPump(1);
        powerDraw = system.getPumpPowerDraw();
        console.log(`  Power draw (one pump): ${powerDraw}W (expected: ${loop1.pumpPowerW}W)`);
        this.assert(powerDraw === loop1.pumpPowerW, 'Power draw updates when pump stopped', 'Should equal single pump power');
        console.log('');
    }
    /**
    ...
    testWarningGeneration() {
        ...
        this.assert(highTempEvent !== undefined, 'High temperature warning generated', 'Should warn at high temperature');
        // Low coolant warning
        system.clearEvents();
        ...
        this.assert(lowCoolantEvent !== undefined, 'Low coolant warning generated', 'Should warn when coolant low');
        ...
        this.assert(pumpFailEvent !== undefined, 'Pump failure warning when empty', 'Should warn when pump runs dry');
        console.log('');
    }
    /**
    ...
    testCoolantRefill() {
        ...
        this.assert(success, 'Refill succeeds', 'Should allow adding coolant');
        this.assert(loop.coolantMassKg === 25, 'Coolant mass increases correctly', 'Should add requested amount');
        // Try to overfill
        ...
        this.assert(loop.coolantMassKg <= loop.maxCapacityKg, 'Cannot overfill beyond capacity', 'Should not exceed maximum');
        console.log('');
    }
    printResults() {
