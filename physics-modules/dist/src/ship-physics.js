...
exports.ShipPhysics = void 0;
class ShipPhysics {
    constructor(config) {
        // Terrain
        this.surfaceElevation = 0; // Terrain elevation at current position (meters)
        // Tracking
     ...
     * Main physics update
     */
    update(dt, mainEngineThrust, // N in body frame
    mainEngineTorque, // N·m from gimbal
    rcsThrust, // N in body frame
    rcsTorque, // N·m from RCS
    propellantConsumed // kg consumed this timestep
    ) {
        // 1. Update mass
        ...
        const totalAccel = this.addVectors(thrustAccel, gravity);
        // 7. Update velocity and position (inertial frame)
        this.velocity = this.addVectors(this.velocity, this.scaleVector(totalAccel, dt));
     ...
     */
    updateRotation(dt, mainEngineTorque, rcsTorque) {
        ...
        this.angularVelocity = this.addVectors(this.angularVelocity, this.scaleVector(angularAccel, dt));
    ...
    checkEvents() {
        ...
        const radarAltitude = altitude - this.surfaceElevation;
        // Ground impact (using terrain elevation)
        if (radarAltitude <= 0) {
            const speed = this.getSpeed();
            this.logEvent(this.simulationTime, 'ground_impact', {
                speed,
                verticalSpeed: this.getVerticalSpeed(),
                terrainElevation: this.surfaceElevation
            });
        }
        // Low altitude warning (radar altitude)
        if (radarAltitude > 0 && radarAltitude < 100) {
    ...
    logEvent(time, type, data) {
        ...
        const recentEvent = this.events.find(e => e.type === type && time - e.time < 1.0);
        if (!recentEvent) {
...
exports.ShipPhysics = ShipPhysics;
//# sourceMappingURL=ship-physics.js.map