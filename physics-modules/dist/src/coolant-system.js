...
exports.CoolantSystem = void 0;
class CoolantSystem {
    constructor(config) {
        // Tracking
        this.totalHeatRejected = 0; // J (total heat radiated to space)
        this.events = [];
        // Constants
        this.STEFAN_BOLTZMANN = 5.67e-8; // W/(m²·K⁴)
        this.COOLANT_DENSITY = 1050; // kg/m³ (water-glycol)
     ...
     * Main update loop
     */
    update(dt, simulationTime, componentTemperatures) {
     ...
     */
    absorbHeatFromComponents(dt, componentTemperatures) {
        for (const loop of this.loops) {
            if (loop.flowRateLPerMin === 0 || loop.coolantMassKg === 0)
                continue;
    ...
    startPump(loopId) {
        ...
        if (!loop)
            return false;
        if (loop.frozen || loop.coolantMassKg === 0)
            return false;
     ...
     */
    createLeak(loopId, leakRateLPerMin) {
        const loop = this.loops[loopId];
        if (loop) {
...
exports.CoolantSystem = CoolantSystem;
//# sourceMappingURL=coolant-system.js.map