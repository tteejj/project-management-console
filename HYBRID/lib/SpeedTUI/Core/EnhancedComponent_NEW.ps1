...
class Component {
    ...
    hidden [RenderEngine]$_renderEngine          # Rendering engine reference
    ...
    Component() {
        ...
        $this._logger.Debug($this._logModule, "Constructor", "Enhanced component created", @{
            Id = $this.Id
    ...
    [void] SetPosition([int]$x, [int]$y) {
        ...
        $this._logger.Trace($this._logModule, "SetPosition", "Position changing", @{
            Id = $this.Id
    ...
    [void] SetSize([int]$width, [int]$height) {
        ...
        # Skip if no change (performance optimization)
        if ($this.Width -eq $width -and $this.Height -eq $height) {
            return
        }
        
        $this._logger.Trace($this._logModule, "SetSize", "Size changing", @{
            Id = $this.Id
        ...
        if ($null -ne $this._renderEngine) {
            $this._renderEngine.RemoveRegion($this._regionId)
            $this._renderEngine.DefineRegion($this._regionId, $this.X, $this.Y, $width, $height)
        }
    ...
    [void] SetTheme([string]$themeName) {
        ...
        $this._logger.Trace($this._logModule, "SetTheme", "Theme changing", @{
            Id = $this.Id
            OldTheme = $this._themeName
            NewTheme = $themeName
        })
    ...
    [void] SetColor([string]$colorRole) {
        ...
        $this._logger.Trace($this._logModule, "SetColor", "Color role changing", @{
            Id = $this.Id
            OldColor = $this._currentColor
            NewColor = $colorRole
        })
    ...
    [void] SetCustomColor([int]$r, [int]$g, [int]$b) {
        ...
        $this._logger.Trace($this._logModule, "SetCustomColor", "Custom color set", @{
            Id = $this.Id
            RGB = "$r,$g,$b"
        })
    ...
    [void] EnableRenderCaching([bool]$enabled) {
        ...
        $this._logger.Debug($this._logModule, "EnableRenderCaching", "Render caching toggled", @{
            Id = $this.Id
            Enabled = $enabled
        })
    }
    ...
    [void] InvalidateRenderCache() {
        if ($this._renderCacheEnabled -and $this._cachedRenderResult.Length -gt 0) {
            $this._cachedRenderResult = ""
            $this._logger.Trace($this._logModule, "InvalidateRenderCache", "Render cache cleared", @{
                Id = $this.Id
            })
        }
    }
    ...
    [void] Initialize([RenderEngine]$renderEngine) {
        $this._logger.Trace($this._logModule, "Initialize", "Initializing component", @{
            Id = $this.Id
            RenderEngine = $renderEngine.GetType().Name
        })
        ...
        try {
            ...
            # Define render region if we have size
            if ($this.Width -gt 0 -and $this.Height -gt 0) {
                $this._renderEngine.DefineRegion($this._regionId, $this.X, $this.Y, $this.Width, $this.Height)
            }
            ...
            $this._logger.Debug($this._logModule, "Initialize", "Component initialized successfully", @{
                Id = $this.Id
    ...
    [void] SetBounds([int]$x, [int]$y, [int]$width, [int]$height) {
        $this._logger.Trace($this._logModule, "SetBounds", "Legacy bounds method called", @{
            Id = $this.Id
    ...
    # Enhanced render method with caching
    [void] Render() {
        if (-not $this.Visible) { 
            return 
        }
    ...
    [void] Invalidate() {
        ...
        if ($null -ne $this._renderEngine) {
            $this._renderEngine.MarkDirty($this._regionId)
        }
    ...
    [void] RemoveChild([Component]$child) {
        ...
        if ($this.Children.Remove($child)) {
            ...
            if ($null -ne $this._renderEngine) {
                $this._renderEngine.RemoveRegion($child._regionId)
            }
    ...
    hidden [void] WriteAt([int]$x, [int]$y, [string]$text) {
        ...
        $this._renderEngine.WriteToRegion($this._regionId, $x, $y, $text)
    }
    
    hidden [void] DrawBox([int]$x, [int]$y, [int]$width, [int]$height) {
        if ($null -eq $this._renderEngine) { return }
        $this._renderEngine.DrawBox($this._regionId, $x, $y, $width, $height)
    }
    
    hidden [void] DrawBox([int]$x, [int]$y, [int]$width, [int]$height, [string]$style) {
        if ($null -eq $this._renderEngine) { return }
        $this._renderEngine.DrawBox($this._regionId, $x, $y, $width, $height, $style)
    }
...
Export-ModuleMember -Function Arrange-Vertically, Arrange-Horizontally