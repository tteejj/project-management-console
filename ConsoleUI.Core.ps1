...
class PmcSimpleTerminal {
    ...
    [void] EndFrame() {
        if ($this.buffering -and $this.buffer.Length -gt 0) {
            # Add VT100 show cursor to the BUFFER (last thing written)
            $this.buffer.Append("`e[?25h") | Out-Null
            # Write entire buffered frame at once - cursor hide/show are IN the buffer
            [Console]::SetCursorPosition(0, 0)
