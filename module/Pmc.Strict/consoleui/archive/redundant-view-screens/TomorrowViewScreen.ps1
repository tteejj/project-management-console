...
class TomorrowViewScreen : PmcScreen {
    ...
    hidden [string] _RenderTaskList() {
        ...
        for ($i = 0; $i -lt $this.TomorrowTasks.Count; $i++) {
            ...
            if ($isEditing -and $this.EditField -eq 'due') {
                $sb.Append($cursorColor)
                $sb.Append(($this.InputBuffer + "_").PadRight(11))
                $sb.Append($reset)
            } elseif ($isSelected) {
                $sb.Append($selectedBg)
                $sb.Append($selectedFg)
            ...
            # Calculate text width for detail lines
        ...
        # Count how many tasks were actually rendered
        $renderedCount = $i
        if ($renderedCount -lt $this.TomorrowTasks.Count -and $currentY -ge ($startY + $maxLines)) {
            $remaining = $this.TomorrowTasks.Count - $renderedCount
            if ($currentY -lt ($startY + $maxLines + 1)) {
