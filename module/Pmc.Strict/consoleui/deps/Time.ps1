...
function ConvertTo-PmcDate {
    <#
    .SYNOPSIS
    Parse flexible date formats: yyyymmdd, mmdd, mm-dd, yyyy-mm-dd, +N, -N, today, tomorrow, yesterday
    ...
    $input = $DateString.Trim().ToLower()

    # Relative dates
    if ($input -eq 'today') { return (Get-Date).ToString('yyyy-MM-dd') }
    if ($input -eq 'tomorrow') { return (Get-Date).AddDays(1).ToString('yyyy-MM-dd') }
    if ($input -eq 'yesterday') { return (Get-Date).AddDays(-1).ToString('yyyy-MM-dd') }

    # +N or -N days from today
    if ($input -match '^[+-](\d+)$') {
        $days = [int]$matches[1]
        if ($input.StartsWith('-')) { $days = -$days }
    ...
    # MMDD format (4 digits) - assume current year
    if ($input -match '^(\d{2})(\d{2})$') {
        try {
            $dt = Get-Date -Year (Get-Date).Year -Month ([int]$matches[1]) -Day ([int]$matches[2])
            return $dt.ToString('yyyy-MM-dd')
        } catch { return $null }
    }

    # MM-DD format (with dash)
    ...
    # Try general DateTime parse as fallback
    try {
...
Export-ModuleMember -Function ConvertTo-PmcDate, Add-PmcTimeEntry, Get-PmcTimeReport, Get-PmcTimeList, Edit-PmcTimeEntry, Remove-PmcTimeEntry, Start-PmcTimer, Stop-PmcTimer, Get-PmcTimerStatus, Show-PmcWeeklyTimeReport
