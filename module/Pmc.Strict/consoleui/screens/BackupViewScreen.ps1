...
class BackupViewScreen : PmcScreen {
    ...
    hidden [void] _SetupMenus() {
        ...
        $tasksMenu.Items.Add([PmcMenuItem]::new("Create Backup", 'C', { $this._CreateBackup() }))
        $tasksMenu.Items.Add([PmcMenuItem]::new("Restore Backup", 'R', { $this._RestoreBackup() }))
        $tasksMenu.Items.Add([PmcMenuItem]::Separator())
        $tasksMenu.Items.Add([PmcMenuItem]::new("Delete Backup", 'D', { $this._DeleteBackup() }))
        $tasksMenu.Items.Add([PmcMenuItem]::Separator())
        $tasksMenu.Items.Add([PmcMenuItem]::new("Refresh", 'F5', { $this.LoadData() }))
    ...
    hidden [void] _RestoreBackup() {
        ...
        # TODO: Navigate to RestoreBackupScreen with selected backup
        $this.ShowStatus("Restore feature: Navigate to RestoreBackupScreen")
    }
