# Task operation handlers for ConsoleUI
...
function Invoke-TaskCopyHandler {
    ...
    try {
        ...
        if (-not $task) {
            ...
            # Get available projects for selection (support string or object entries)
            $projectList = @('(none)', 'inbox') + @(
                $data.projects |
                    ForEach-Object {
                        if ($_ -is [string]) { $_ }
                        elseif ($_.PSObject.Properties['name']) { $_.name }
                    } |
                    Where-Object { $_ -and $_ -ne 'inbox' } |
                    Sort-Object -Unique
            )
...
function Invoke-TaskPostponeHandler {
    ...
    try {
        ...
        if (-not $task) {
            ...
            # Read current due from 'due' or legacy 'dueDate'
            $currentDue = $null
            if ($task.PSObject.Properties['due'] -and $task.due) { try { $currentDue = [datetime]$task.due } catch {} }
            elseif ($task.PSObject.Properties['dueDate'] -and $task.dueDate) { try { $currentDue = [datetime]$task.dueDate } catch {} }

            $newDue = if ($currentDue) { $currentDue.AddDays($days) } else { (Get-Date).AddDays($days) }
            $newDueStr = $newDue.ToString("yyyy-MM-dd")
            # Write back to canonical 'due' and also legacy 'dueDate' for compatibility
            $task | Add-Member -MemberType NoteProperty -Name 'due' -Value $newDueStr -Force
            if ($task.PSObject.Properties['dueDate']) { $task.dueDate = $newDueStr }

            Save-PmcData -Data $data -Action "Postponed task #$id by $days days"
            Show-InfoMessage -Message "Task postponed! New due: $newDueStr" -Title "Success" -Color "Green"
        }
    } catch {
        Show-InfoMessage -Message "Error: $_" -Title "Error" -Color "Red"
    }
