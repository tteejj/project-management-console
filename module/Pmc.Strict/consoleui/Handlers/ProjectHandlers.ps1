# Project operation handlers for ConsoleUI
...
function Invoke-ProjectInfoHandler {
    ...
    try {
        ...
        # Consider both 'due' and legacy 'dueDate' properties
        $overdueTasks = @($activeTasks | Where-Object {
            $d = $null
            if ($_.PSObject.Properties['due'] -and $_.due) { $d = $_.due }
            elseif ($_.PSObject.Properties['dueDate'] -and $_.dueDate) { $d = $_.dueDate }
            if ($null -eq $d) { return $false }
            try { ([datetime]$d).Date -lt (Get-Date).Date } catch { $false }
        })
