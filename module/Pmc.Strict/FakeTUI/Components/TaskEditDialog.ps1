    ...
    [void] SaveTask() {
        try {
            ...
            if ($this.IsNewTask) {
                ...
                $task = $data.tasks | Where-Object { $_.id -eq $this.Task.id } | Select-Object -First 1
                if ($task) {
                    $task.text = $this.Fields.text.Value
                    $task.status = $this.Fields.status.Value
                    $task.priority = $this.Fields.priority.Value
                    $task.project = $this.Fields.project.Value
                    $task.modified = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')

                    # Process tags
                    $task.tags = if ($this.Fields.tags.Value) {
                    ...
                    # Process notes
                    $task.notes = if ($this.Fields.notes.Value) {
            ...
            Save-StrictData $data "task edit via GUI"
