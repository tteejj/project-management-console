    ...
    [string] HandleInput() {
        while ($true) {
            ...
            if (-not $this.inMenuMode) {
                ...
                if ($key.Modifiers -eq 'Alt' -or $key.Key -eq 'F10') {
                    # Enter menu mode
                ...
                if ($key.Modifiers -eq 'Alt' -and [char]::IsLetter($key.KeyChar)) {
                    $keyChar = [char]::ToUpper($key.KeyChar)
                    for ($i = 0; $i -lt $this.menuOrder.Count; $i++) {
                        $menuName = $this.menuOrder[$i]
                ...
                if ($key.Key -eq 'Escape' -or ($key.Key -eq 'Q' -and $key.Modifiers -eq 'Control')) {
                    return "app:exit"
                }
