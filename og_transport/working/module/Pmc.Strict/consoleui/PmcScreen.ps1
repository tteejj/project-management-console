...
class PmcScreen {
    ...
    [void] OnDoExit() {
        if ($global:PmcTuiLogFile) {
            Add-Content -Path $global:PmcTuiLogFile -Value "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss.fff')] [LIFECYCLE] PmcScreen.OnDoExit() - Screen='$($this.ScreenKey)' exiting"
        }
        $this.IsActive = $false
        ...
        if ($global:PmcTuiLogFile) {
            Add-Content -Path $global:PmcTuiLogFile -Value "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss.fff')] [LIFECYCLE] PmcScreen.OnDoExit() - Screen='$($this.ScreenKey)' exited successfully"
        }
    }
    ...
    [string] Render() {
        ...
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts >>>>>>> PmcScreen.Render START for screen=$($this.GetType().Name)"
        $sb = [System.Text.StringBuilder]::new(4096)
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   StringBuilder created, initial capacity=4096"
        ...
        if ($this.MenuBar) {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Rendering MenuBar..."
            if ($global:PmcTuiLogFile) {
            ...
            if ($output) {
                ...
                $afterLen = $sb.Length
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   MenuBar appended $($output.Length) chars, sb.Length $beforeLen -> $afterLen"
                if ($global:PmcTuiLogFile) {
                    Add-Content -Path $global:PmcTuiLogFile -Value "[$ts] PmcScreen.Render: MenuBar output length=$($output.Length)"
                }
            } else {
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   MenuBar returned null/empty"
                if ($global:PmcTuiLogFile) {
            ...
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   No MenuBar to render"
        }

        # Render Header
        if ($this.Header) {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Rendering Header..."
            ...
            if ($output) {
                ...
                $afterLen = $sb.Length
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Header appended $($output.Length) chars, sb.Length $beforeLen -> $afterLen"
            } else {
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Header returned null/empty"
            }
        } else {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   No Header to render"
        }

        # Render content (override in subclass) - wrap in try-catch to prevent rendering crashes
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Calling RenderContent() on $($this.GetType().Name)..."
        if ($global:PmcTuiLogFile) {
        ...
        try {
            ...
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   RenderContent() returned, output length=$($contentOutput.Length)"
            if ($contentOutput) {
                $sb.Append($contentOutput)
                $afterLen = $sb.Length
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Content appended $($contentOutput.Length) chars, sb.Length $beforeLen -> $afterLen"
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Content first 200 chars - $($contentOutput.Substring(0, [Math]::Min(200, $contentOutput.Length)).Replace("`e", '<ESC>'))"
                if ($contentOutput.Length -gt 400) {
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Content chars 400-600 - $($contentOutput.Substring(400, [Math]::Min(200, $contentOutput.Length - 400)).Replace("`e", '<ESC>'))"
                }
                if ($global:PmcTuiLogFile) {
                ...
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   RenderContent() returned null/empty"
                if ($global:PmcTuiLogFile) {
            ...
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ERROR - $errorMsg"
            if ($global:PmcTuiLogFile) {
        ...
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Rendering $widgetCount ContentWidgets..."
        $widgetIndex = 0
        foreach ($widget in $this.ContentWidgets) {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Rendering ContentWidget[$widgetIndex] type=$($widget.GetType().Name)..."
            ...
            if ($output) {
                ...
                $afterLen = $sb.Length
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ContentWidget[$widgetIndex] appended $($output.Length) chars, sb.Length $beforeLen -> $afterLen"
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ContentWidget[$widgetIndex] first 100 chars - $($output.Substring(0, [Math]::Min(100, $output.Length)).Replace("`e", '<ESC>'))"
            } else {
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ContentWidget[$widgetIndex] returned null/empty"
            }
            $widgetIndex++
        }
        ...
        if ($this.StatusBar) {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Rendering StatusBar..."
            ...
            if ($output) {
                ...
                $afterLen = $sb.Length
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   StatusBar appended $($output.Length) chars, sb.Length $beforeLen -> $afterLen"
            } else {
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   StatusBar returned null/empty"
            }
        } else {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   No StatusBar to render"
        }

        $result = $sb.ToString()
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Final result length=$($result.Length)"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Final first 200 chars - $($result.Substring(0, [Math]::Min(200, $result.Length)).Replace("`e", '<ESC>'))"
        if ($result.Length -gt 400) {
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Final chars 400-600 - $($result.Substring(400, [Math]::Min(200, $result.Length - 400)).Replace("`e", '<ESC>'))"
        }
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts <<<<<<< PmcScreen.Render END, returning $($result.Length) chars"
        return $result
    }
    ...
    [void] RenderToEngine([object]$engine) {
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts ======== RenderToEngine START for screen=$($this.GetType().Name) ========"
        ...
        # Layer 50: Header
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   BeginLayer(Header)..."
        $engine.BeginLayer([ZIndex]::Header)
        if ($this.Header) {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Rendering Header to engine..."
            try {
                $output = $this.Header.Render()
                if ($output) {
                    $ts = Get-Date -Format 'HH:mm:ss.fff'
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Header rendered $($output.Length) chars, calling _ParseAnsiAndWrite..."
                    $this._ParseAnsiAndWrite($engine, $output)
                    $ts = Get-Date -Format 'HH:mm:ss.fff'
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Header _ParseAnsiAndWrite complete"
                }
            } catch {
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ERROR rendering Header - $_"
        ...
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   BeginLayer(Content) layer=10..."
        ...
        try {
            ...
            if ($this.PSObject.Methods['RenderContentToEngine'] -and
                ...
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Using RenderContentToEngine path (direct engine)"
                if ($global:PmcTuiLogFile) {
                ...
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   RenderContentToEngine complete"
            } else {
                # Fallback: Use ANSI string rendering (legacy path)
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Using RenderContent path (ANSI string fallback)"
                if ($global:PmcTuiLogFile) {
                    Add-Content -Path $global:PmcTuiLogFile -Value "[$ts] PmcScreen.RenderToEngine: Using RenderContent path (fallback)"
                }
                $contentOutput = $this.RenderContent()
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   RenderContent() returned $($contentOutput.Length) chars"
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Content first 200 chars - $($contentOutput.Substring(0, [Math]::Min(200, $contentOutput.Length)).Replace("`e", '<ESC>'))"
                if ($contentOutput.Length -gt 400) {
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Content chars 400-600 - $($contentOutput.Substring(400, [Math]::Min(200, $contentOutput.Length - 400)).Replace("`e", '<ESC>'))"
                }
                ...
                if ($contentOutput) {
                    $ts = Get-Date -Format 'HH:mm:ss.fff'
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Calling _ParseAnsiAndWrite with $($contentOutput.Length) chars..."
                    $this._ParseAnsiAndWrite($engine, $contentOutput)
                    $ts = Get-Date -Format 'HH:mm:ss.fff'
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   _ParseAnsiAndWrite complete"
                }
            }
        } catch {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ERROR rendering content - $_"
            if ($global:PmcTuiLogFile) {
        ...
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   BeginLayer(Panel) layer=20, ContentWidgets.Count=$($this.ContentWidgets.Count)..."
        ...
        foreach ($widget in $this.ContentWidgets) {
            ...
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Rendering ContentWidget[$widgetIndex] name=$widgetName type=$($widget.GetType().Name)..."
            try {
                $output = $widget.Render()
                ...
                if ($output -and $output.Length -gt 0) {
                    $ts = Get-Date -Format 'HH:mm:ss.fff'
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ContentWidget[$widgetIndex] rendered $($output.Length) chars, calling _ParseAnsiAndWrite..."
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ContentWidget[$widgetIndex] first 100 chars - $($output.Substring(0, [Math]::Min(100, $output.Length)).Replace("`e", '<ESC>'))"
                    $this._ParseAnsiAndWrite($engine, $output)
                    $ts = Get-Date -Format 'HH:mm:ss.fff'
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ContentWidget[$widgetIndex] _ParseAnsiAndWrite complete"
                } else {
                    $ts = Get-Date -Format 'HH:mm:ss.fff'
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ContentWidget[$widgetIndex] returned null/empty (fast path)"
                }
                # else: widget used fast path (OnRenderToEngine), already rendered
            } catch {
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ERROR rendering ContentWidget[$widgetIndex] - $_"
        ...
        # Layer 55: Footer
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   BeginLayer(Footer) layer=55..."
        $engine.BeginLayer([ZIndex]::Footer)
        if ($this.Footer) {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Rendering Footer..."
            try {
                $output = $this.Footer.Render()
                if ($output) {
                    $ts = Get-Date -Format 'HH:mm:ss.fff'
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Footer rendered $($output.Length) chars"
                    $this._ParseAnsiAndWrite($engine, $output)
                }
            } catch {
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ERROR rendering Footer - $_"
        ...
        # Layer 65: StatusBar
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   BeginLayer(StatusBar) layer=65..."
        ...
        if ($this.StatusBar) {
            ...
            try {
                ...
                if ($output) {
                    $ts = Get-Date -Format 'HH:mm:ss.fff'
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   StatusBar rendered $($output.Length) chars"
                    $this._ParseAnsiAndWrite($engine, $output)
                }
            } catch {
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ERROR rendering StatusBar - $_"
        ...
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   BeginLayer(Dropdown) layer=100 (MenuBar LAST for z-order)..."
        $engine.BeginLayer([ZIndex]::Dropdown)
        if ($this.MenuBar) {
            $ts = Get-Date -Format 'HH:mm:ss.fff'
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Rendering MenuBar (dropdown layer)..."
            try {
                if ($global:PmcTuiLogFile) {
    ...
    hidden [void] _ParseAnsiAndWrite([object]$engine, [string]$ansiOutput) {
        ...
        for ($i = 0; $i -lt $matches.Count; $i++) {
            ...
            if ($y -eq 12 -and $x -ge 6 -and $x -le 8) {
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   _ParseAnsiAndWrite Match $i - WriteAt($x,$y) content_length=$($content.Length)"
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Content: $($content.Replace("`e", '<ESC>'))"
                if ($content.Contains("`e[30;47m")) {
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   *** HIGHLIGHT CODE FOUND in content! ***"
                } else {
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   *** NO HIGHLIGHT CODE in content ***"
                }
            }
            ...
            if ($content) {
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Calling engine.WriteAt($x, $y, [content $($content.Length) chars])..."
                $engine.WriteAt($x, $y, $content)
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   engine.WriteAt complete"
            }
        }
    }
