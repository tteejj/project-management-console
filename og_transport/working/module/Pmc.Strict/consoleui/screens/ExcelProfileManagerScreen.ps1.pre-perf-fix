...
class ExcelProfileManagerScreen : StandardListScreen {
    ...
    ExcelProfileManagerScreen() : base("ExcelProfiles", "Excel Import Profiles") {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') ========== EXCELPROFILEMANAGERSCREEN CONSTRUCTOR CALLED =========="

        # Initialize service
        $this._mappingService = [ExcelMappingService]::GetInstance()
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN: Got mappingService"
    ...
    ExcelProfileManagerScreen([object]$container) : base("ExcelProfiles", "Excel Import Profiles", $container) {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') ========== EXCELPROFILEMANAGERSCREEN CONTAINER CONSTRUCTOR CALLED =========="
    ...
    [array] GetColumns() {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.GetColumns: CALLED"
        return @(
    ...
    [void] LoadData() {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.LoadData: CALLED"
        try {
            $items = $this.LoadItems()
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.LoadData: Got $($items.Count) items from LoadItems()"
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.LoadData: About to call List.SetData"
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.LoadData: List is null? $($null -eq $this.List)"
            $this.List.SetData($items)
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.LoadData: SetData complete"
        } catch {
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.LoadData: EXCEPTION - $($_.Exception.Message)"
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.LoadData: STACK - $($_.ScriptStackTrace)"
            throw
        }
    }
    ...
    [array] LoadItems() {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.LoadItems: START"
        $profiles = @($this._mappingService.GetAllProfiles())
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELPROFILEMANAGERSCREEN.LoadItems: Got $($profiles.Count) profiles from service"
