...
class ExcelImportScreen : PmcScreen {
    ...
    ExcelImportScreen() : base("ExcelImport", "Import from Excel") {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') ========== EXCELIMPORTSCREEN CONSTRUCTOR CALLED =========="

        try {
            $this._reader = [ExcelComReader]::new()
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELIMPORTSCREEN: ExcelComReader created successfully"
        } catch {
            $this._errorMessage = "Excel COM not available: $($_.Exception.Message). Excel must be installed to use this feature."
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELIMPORTSCREEN: FAILED to create ExcelComReader - $($_.Exception.Message)"
    ...
    ExcelImportScreen([object]$container) : base("ExcelImport", "Import from Excel", $container) {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') ========== EXCELIMPORTSCREEN CONTAINER CONSTRUCTOR CALLED =========="
    ...
    [string] Render() {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELIMPORTSCREEN.Render: CALLED step=$($this._step)"
        ...
        switch ($this._step) {
            1 {
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELIMPORTSCREEN.Render: Calling _RenderStep1"
                $this._RenderStep1($sb, $y, $contentWidth)
            }
            2 {
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELIMPORTSCREEN.Render: Calling _RenderStep2"
                $this._RenderStep2($sb, $y, $contentWidth)
            }
            3 {
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELIMPORTSCREEN.Render: Calling _RenderStep3"
                $this._RenderStep3($sb, $y, $contentWidth)
            }
            4 {
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EXCELIMPORTSCREEN.Render: Calling _RenderStep4"
