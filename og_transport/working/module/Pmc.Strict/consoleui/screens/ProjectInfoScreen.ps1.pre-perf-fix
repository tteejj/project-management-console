...
class ProjectInfoScreen : PmcScreen {
    ...
    [string] RenderContent() {
        try {
            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') RenderContent START *** NEW CODE VERSION 2.0 ***"
            ...
            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') Getting content area"
            # Get content area
        ...
        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') INITIALIZED CURSOR TRACKING: editingCursorRow=$editingCursorRow editingCursorCol=$editingCursorCol"
        ...
        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') Rendering project name"
        $sb.Append($this.Header.BuildMoveTo($contentRect.X + 4, $y++))
        $sb.Append($highlightColor)
        $name = $(if ($this.ProjectData -is [string]) { $this.ProjectData } else { Get-SafeProperty $this.ProjectData 'name' })
        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') name=$name type=$($name.GetType().FullName)"
        ...
        if ($this.ProjectData -isnot [string]) {
            ...
            if ($this.EditMode) {
                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EDIT MODE: EditableFields.Count=$($this.EditableFields.Count) SelectedFieldIndex=$($this.SelectedFieldIndex) IsEditingField=$($this.IsEditingField)"
                ...
                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') Column positions: col1X=$col1X col2X=$col2X col3X=$col3X contentRect.Width=$($contentRect.Width)"
                ...
                for ($i = 0; $i -lt $this.EditableFields.Count; $i += 3) {
                    if ($i -lt 9) {  # Log first 3 rows
                        ...
                        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') Row i=$i y=$y Labels: [$f1Label] [$f2Label] [$f3Label]"
                    }
                    ...
                    if ($i -lt 9) {
                        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 i=$i SelectedFieldIndex=$($this.SelectedFieldIndex) isSelected1=$isSelected1 Label=$($field1.Label)"
                    }

                    if ($i -lt 6 -and $isSelected1) {
                        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 SELECTED: i=$i field=$($field1.Label)"
                    }

                    if ($i -eq 0) {
                        Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') === COL1 START i=$i y=$y isSelected1=$isSelected1 IsEditingField=$($this.IsEditingField) ==="
                    }
                    ...
                    if ($i -eq 0) {
                        ...
                        Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After MoveTo($col1X,$y): last30='$($tail -replace "`e", '<ESC>')'"
                    }
                    ...
                    if ($isSelected1) {
                        if ($i -eq 0) {
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 RENDERING SELECTED: selectColor='$selectColor' highlightColor='$highlightColor'"
                        }
                        $sb.Append($selectColor)
                        if ($i -eq 0) {
                            $curr = $sb.ToString()
                            $tail = $(if ($curr.Length -gt 30) { $curr.Substring($curr.Length - 30) } else { $curr })
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After selectColor: last30='$($tail -replace "`e", '<ESC>')'"
                        }
                        $sb.Append($label1)
                        if ($i -eq 0) {
                            $curr = $sb.ToString()
                            $tail = $(if ($curr.Length -gt 30) { $curr.Substring($curr.Length - 30) } else { $curr })
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After label '$label1': last30='$($tail -replace "`e", '<ESC>')'"
                        }
                        $sb.Append(' ' * (22 - $label1.Length))  # Pad WITH selectColor still active
                        if ($i -eq 0) {
                            $curr = $sb.ToString()
                            $tail = $(if ($curr.Length -gt 30) { $curr.Substring($curr.Length - 30) } else { $curr })
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After padding: last30='$($tail -replace "`e", '<ESC>')'"
                        }
                        ...
                        if (-not ($this.IsEditingField -and $isSelected1)) {
                            $sb.Append($highlightColor)
                            if ($i -eq 0) {
                                Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After highlightColor (NOT editing): last30='$(($sb.ToString() | Select-Object -Last 1).Substring([Math]::Max(0, ($sb.ToString()).Length - 30)) -replace "`e", '<ESC>')'"
                            }
                        } else {
                            if ($i -eq 0) {
                                Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') SKIPPED highlightColor (IS editing)"
                            }
                        }
                    } else {
                        if ($i -eq 1) {
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 RENDERING NOT SELECTED: mutedColor='$mutedColor' textColor='$textColor'"
                        }
                        $sb.Append($mutedColor)
                        $sb.Append($label1)
                        $sb.Append($reset)
                    ...
                    if ($this.IsEditingField -and $isSelected1) {
                        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 SHOWING CURSOR: EditingValue='$($this.EditingValue)' i=$i y=$y col1X=$col1X"
                        if ($i -eq 0) {
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') EDITING - about to append highlightColor then value then cursor"
                        }
                        $sb.Append($highlightColor)  # Set color for value
                        if ($i -eq 0) {
                            $curr = $sb.ToString()
                            $tail = $(if ($curr.Length -gt 40) { $curr.Substring($curr.Length - 40) } else { $curr })
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After highlightColor: last40='$($tail -replace "`e", '<ESC>')'"
                        }
                        $sb.Append($this.EditingValue)
                        if ($i -eq 0) {
                            $curr = $sb.ToString()
                            $tail = $(if ($curr.Length -gt 40) { $curr.Substring($curr.Length - 40) } else { $curr })
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After EditingValue='$($this.EditingValue)': last40='$($tail -replace "`e", '<ESC>')'"
                        }
                        ...
                        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 STORED CURSOR POSITION: editingCursorRow=$editingCursorRow editingCursorCol=$editingCursorCol (calculated: col1X=$col1X + 22 + len=$($this.EditingValue.Length))"
                        # NOW append the visual cursor block
                        $sb.Append("`e[7m `e[0m")  # Block cursor with reset
                        if ($i -eq 0) {
                            $curr = $sb.ToString()
                            $tail = $(if ($curr.Length -gt 40) { $curr.Substring($curr.Length - 40) } else { $curr })
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After cursor: last40='$($tail -replace "`e", '<ESC>')'"
                        }
                    } elseif ($field1.Value) {
                    ...
                    if ($i -eq 0) {
                        ...
                        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   AFTER COL1 RESET: last100='$($last100 -replace "`e", '<ESC>')'"
                        $curr = $sb.ToString()
                        $tail = $(if ($curr.Length -gt 50) { $curr.Substring($curr.Length - 50) } else { $curr })
                        Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After final COL1 reset: last50='$($tail -replace "`e", '<ESC>')'"
                        Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') === COL1 END ==="
                    }
                    ...
                    if ($i + 1 -lt $this.EditableFields.Count) {
                        ...
                        if ($i -lt 9) {
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 i=$($i+1) SelectedFieldIndex=$($this.SelectedFieldIndex) isSelected2=$isSelected2 Label=$($field2.Label)"
                        }

                        if ($i -eq 0) {
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') === COL2 START i=$($i+1) y=$y isSelected2=$isSelected2 ==="
                        }
                        ...
                        if ($i -eq 0) {
                            ...
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   BEFORE COL2 MOVETO: last50='$($last50 -replace "`e", '<ESC>')'"
                            $curr = $sb.ToString()
                            $tail = $(if ($curr.Length -gt 40) { $curr.Substring($curr.Length - 40) } else { $curr })
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') Before COL2 reset: last40='$($tail -replace "`e", '<ESC>')'"
                        }
                        ...
                        if ($i -eq 0) {
                            ...
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   AFTER COL2 MOVETO: last50='$($last50 -replace "`e", '<ESC>')'"
                            $curr = $sb.ToString()
                            $tail = $(if ($curr.Length -gt 40) { $curr.Substring($curr.Length - 40) } else { $curr })
                            Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After COL2 MoveTo($col2X,$y): last40='$($tail -replace "`e", '<ESC>')'"
                        }
                        ...
                        if ($isSelected2) {
                            if ($i -eq 0) {
                                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 RENDERING SELECTED: selectColor='$selectColor' highlightColor='$highlightColor'"
                            }
                            $sb.Append($selectColor)
                            $sb.Append($label2)
                            ...
                            if ($i -eq 0) {
                                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 RENDERING NOT SELECTED: mutedColor='$mutedColor' textColor='$textColor'"
                                Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') COL2 NOT SELECTED - appending mutedColor for label"
                            }
                            $sb.Append($mutedColor)
                            if ($i -eq 0) {
                                $curr = $sb.ToString()
                                $tail = $(if ($curr.Length -gt 30) { $curr.Substring($curr.Length - 30) } else { $curr })
                                Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After mutedColor: last30='$($tail -replace "`e", '<ESC>')'"
                            }
                            $sb.Append($label2)
                            if ($i -eq 0) {
                                $curr = $sb.ToString()
                                $tail = $(if ($curr.Length -gt 30) { $curr.Substring($curr.Length - 30) } else { $curr })
                                Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After label2 '$label2': last30='$($tail -replace "`e", '<ESC>')'"
                            }
                            $sb.Append($reset)
                            if ($i -eq 0) {
                                $curr = $sb.ToString()
                                $tail = $(if ($curr.Length -gt 30) { $curr.Substring($curr.Length - 30) } else { $curr })
                                Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After reset: last30='$($tail -replace "`e", '<ESC>')'"
                            }
                            ...
                            if ($i -eq 0) {
                                $curr = $sb.ToString()
                                $tail = $(if ($curr.Length -gt 30) { $curr.Substring($curr.Length - 30) } else { $curr })
                                Add-Content -Path "$($env:TEMP)\pmc-col-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') After textColor: last30='$($tail -replace "`e", '<ESC>')'"
                            }
                        }

                        if ($this.IsEditingField -and $isSelected2) {
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 SHOWING CURSOR: EditingValue='$($this.EditingValue)' i=$($i+1) y=$y col2X=$col2X"
                            ...
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 STORED CURSOR POSITION: editingCursorRow=$editingCursorRow editingCursorCol=$editingCursorCol (calculated: col2X=$col2X + 22 + len=$($this.EditingValue.Length))"
                        } elseif ($field2.Value) {
                            # Show saved value (truncate if needed)
                    ...
                    if ($i + 2 -lt $this.EditableFields.Count) {
                        ...
                        if ($i -lt 9) {
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 i=$($i+2) SelectedFieldIndex=$($this.SelectedFieldIndex) isSelected3=$isSelected3 Label=$($field3.Label)"
                        }
                        ...
                        if ($this.IsEditingField -and $isSelected3) {
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 SHOWING CURSOR: EditingValue='$($this.EditingValue)' i=$($i+2) y=$y col3X=$col3X"
                            # DON'T use BuildMoveTo - just let it flow naturally after the label
                            $sb.Append($highlightColor)  # Set color for value
                            $sb.Append($this.EditingValue)
                            $sb.Append("`e[7m `e[0m")  # Block cursor with reset
                            ...
                            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 STORED CURSOR POSITION: editingCursorRow=$editingCursorRow editingCursorCol=$editingCursorCol (calculated: col3X=$col3X + 22 + len=$($this.EditingValue.Length))"
                        } elseif ($field3.Value) {
                            # Show saved value (truncate if needed)
        ...
        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') Rendering stats section"
        $y++
        $sb.Append($this.Header.BuildMoveTo($contentRect.X + 4, $y++))
        $sb.Append($mutedColor)
        $sb.Append("Tasks: ")
        $sb.Append($reset)
        $sb.Append($textColor)
        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') About to append TotalTasks=$($this.ProjectStats.TotalTasks) type=$($this.ProjectStats.TotalTasks.GetType().FullName)"
        ...
        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') CURSOR CHECK: EditMode=$($this.EditMode) IsEditingField=$($this.IsEditingField) editingCursorRow=$editingCursorRow editingCursorCol=$editingCursorCol"
        if ($this.EditMode -and $this.IsEditingField -and $null -ne $editingCursorRow -and $null -ne $editingCursorCol) {
            ...
            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') REPOSITIONING CURSOR: moveSeq='$($moveSeq -replace "`e", '<ESC>')' (0-based: col=$editingCursorCol row=$editingCursorRow)"
            $sb.Append($moveSeq)
            # Also make cursor visible
            $sb.Append("`e[?25h")
            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') REPOSITIONED CURSOR to row=$editingCursorRow col=$editingCursorCol (VT100: $($moveSeq -replace "`e", '<ESC>'))"
        }
        ...
        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') RenderContent SUCCESS"
        return $finalString
        } catch {
            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') ERROR at line $($_.InvocationInfo.ScriptLineNumber): $($_.Exception.Message)"
            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') StackTrace: $($_.ScriptStackTrace)"
            throw
        }
    }
    ...
    [bool] HandleKeyPress([ConsoleKeyInfo]$keyInfo) {
        ...
        if ($this.EditMode) {
            if ($key -eq [ConsoleKey]::UpArrow) {
                ...
                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') UP ARROW: SelectedFieldIndex=$($this.SelectedFieldIndex)"
                if ($this.SelectedFieldIndex -ge 3) {
                    $this.SelectedFieldIndex -= 3
                    Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') UP ARROW: New SelectedFieldIndex=$($this.SelectedFieldIndex)"
                }
                return $true
            }
            elseif ($key -eq [ConsoleKey]::DownArrow) {
                # Move down 3 positions (one row down in 3-column layout)
                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') DOWN ARROW: SelectedFieldIndex=$($this.SelectedFieldIndex)"
                if ($this.SelectedFieldIndex + 3 -lt $this.EditableFields.Count) {
                    $this.SelectedFieldIndex += 3
                    Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') DOWN ARROW: New SelectedFieldIndex=$($this.SelectedFieldIndex)"
                }
                return $true
            }
            elseif ($key -eq [ConsoleKey]::LeftArrow) {
                # Move to previous field (left in row)
                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') LEFT ARROW: SelectedFieldIndex=$($this.SelectedFieldIndex)"
                if ($this.SelectedFieldIndex -gt 0) {
                    $this.SelectedFieldIndex--
                    Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') LEFT ARROW: New SelectedFieldIndex=$($this.SelectedFieldIndex)"
                }
                return $true
            }
            elseif ($key -eq [ConsoleKey]::RightArrow) {
                # Move to next field (right in row)
                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') RIGHT ARROW: SelectedFieldIndex=$($this.SelectedFieldIndex)"
                if ($this.SelectedFieldIndex -lt ($this.EditableFields.Count - 1)) {
                    $this.SelectedFieldIndex++
                    Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') RIGHT ARROW: New SelectedFieldIndex=$($this.SelectedFieldIndex)"
                }
                return $true
            }
            elseif ($key -eq [ConsoleKey]::Enter) {
                # Start editing the selected field
                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') ENTER PRESSED: SelectedFieldIndex=$($this.SelectedFieldIndex)"
                ...
                $this._UpdateFooterShortcuts()
                Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') STARTED EDITING: Field=$($selectedField.Name) InitialValue='$($this.EditingValue)' IsEditingField=$($this.IsEditingField)"
                return $true
            }
            elseif ($keyChar -eq 'e') {
    ...
    hidden [void] _EditProject() {
        Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') _EditProject CALLED: EditMode=$($this.EditMode)"
        if ($this.EditMode) {
            ...
            # Enter edit mode
            Add-Content -Path "$($env:TEMP)\pmc-project-render.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') _EditProject: ENTERING EDIT MODE, resetting SelectedFieldIndex to 0"
            $this.EditMode = $true
