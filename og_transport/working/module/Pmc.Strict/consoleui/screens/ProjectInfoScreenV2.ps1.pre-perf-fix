...
class ProjectInfoScreenV2 : StandardListScreen {
    ...
    hidden [void] _InitializeScreen() {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** ProjectInfoScreenV2._InitializeScreen CALLED - CLASS LOADED ***"
        ...
        if ($this.List) {
            ...
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   List.Visible set to FALSE to prevent overlap with custom grid"
        }
    ...
    [void] SetProject([string]$projectName) {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** SetProject CALLED with projectName='$projectName' ***"
        ...
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** SetProject END ***"
    }

    # Override OnEnter to load data
    [void] OnEnter() {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** OnEnter START - ProjectName='$($this.ProjectName)' ***"
        if ($global:PmcTuiLogFile) {
        ...
        $this.IsActive = $true
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   IsActive set to true"

        # CRITICAL: Ensure InlineEditor is hidden on entry
        $this.ShowInlineEditor = $false
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   ShowInlineEditor=$($this.ShowInlineEditor)"

        # Set columns
        $columns = $this.GetColumns()
        $this.List.SetColumns($columns)
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Columns set: $($columns.Count) columns"

        # Load data (don't call parent's LoadData - it expects different data structure)
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Calling LoadData..."
        $this.LoadData()
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   LoadData complete - AllFields.Count=$($this.AllFields.Count)"
        ...
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** OnEnter END ***"
    }

    [void] LoadData() {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** LoadData START - ProjectName='$($this.ProjectName)' ***"
        if ($global:PmcTuiLogFile) {
        ...
        if ([string]::IsNullOrWhiteSpace($this.ProjectName)) {
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   ABORT - ProjectName is empty"
        ...
        try {
            ...
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Getting all projects from Store..."
            $allProjects = $this.Store.GetAllProjects()
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Got $($allProjects.Count) projects"
            if ($global:PmcTuiLogFile) {
    ...
    hidden [void] _BuildFieldList() {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** _BuildFieldList START - ProjectData=$($null -ne $this.ProjectData) ***"
        ...
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** _BuildFieldList END - Built $($this.AllFields.Count) fields ***"
        if ($this.AllFields.Count -gt 0) {
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   First 3 fields: $($this.AllFields[0].Label)='$($this.AllFields[0].Value)' | $($this.AllFields[1].Label)='$($this.AllFields[1].Value)' | $($this.AllFields[2].Label)='$($this.AllFields[2].Value)'"
        }
    }

    # Override RenderContent to render 3-column grid (EXACTLY matching original)
    [string] RenderContent() {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') ========================================="
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** ProjectInfoScreenV2.RenderContent START ***"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   AllFields.Count=$($this.AllFields.Count)"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   ProjectData=$($null -ne $this.ProjectData)"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   ProjectName='$($this.ProjectName)'"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   EditMode=$($this.EditMode)"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   ShowInlineEditor=$($this.ShowInlineEditor)"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   List=$($null -ne $this.List)"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   LayoutManager=$($null -ne $this.LayoutManager)"
        ...
        if (-not $this.LayoutManager) {
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** ABORT - NO LAYOUT MANAGER! ***"
            return $sb.ToString()
        }

        # Get content area
        $contentRect = $this.LayoutManager.GetRegion('Content', $this.TermWidth, $this.TermHeight)
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   contentRect.X=$($contentRect.X) Y=$($contentRect.Y) W=$($contentRect.Width) H=$($contentRect.Height)"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   TermWidth=$($this.TermWidth) TermHeight=$($this.TermHeight)"
        ...
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Checking AllFields.Count > 0: $($this.AllFields.Count -gt 0)"
        if ($this.AllFields.Count -gt 0) {
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   RENDERING 3-COLUMN GRID with $($this.AllFields.Count) fields"
            $y++
            ...
            $col2X = $col1X + 42
            $col3X = $col2X + 42
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Column positions: col1X=$col1X col2X=$col2X col3X=$col3X"

            # Render fields in rows of 3
            $fieldIndex = 0
            $rowY = $y + 1
            $rowsRendered = 0
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Starting grid render loop, initial rowY=$rowY"

            while ($fieldIndex -lt $this.AllFields.Count) {
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') === ROW $rowsRendered START: fieldIndex=$fieldIndex rowY=$rowY ==="
                ...
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ROW $rowsRendered - field1=$(if($field1){$field1.Label}else{'null'}) field2=$(if($field2){$field2.Label}else{'null'}) field3=$(if($field3){$field3.Label}else{'null'})"
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ROW $rowsRendered - values='$(if($field1){$field1.Value}else{'N/A'})' '$(if($field2){$field2.Value}else{'N/A'})' '$(if($field3){$field3.Value}else{'N/A'})'"
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ROW $rowsRendered - CurrentSelectedIndex=$($this.List.GetSelectedIndex())"

                # Render COL1
                if ($field1) {
                    $isSelected1 = ($fieldIndex -eq $this.List.GetSelectedIndex())
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 render start: row=$rowsRendered fieldIdx=$fieldIndex label=$($field1.Label) isSelected=$isSelected1 ShowInlineEditor=$($this.ShowInlineEditor)"

                    $moveToCode = $this.Header.BuildMoveTo($col1X, $rowY)
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 MoveTo: col1X=$col1X rowY=$rowY code='$($moveToCode.Replace("`e", '<ESC>'))'"
                    ...
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 label truncated: '$label1' (original='$($field1.Label)')"

                    # Render label with selection highlight when selected (but not editing)
                    if ($isSelected1 -and -not $this.ShowInlineEditor) {
                        # Show selection highlight
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 BRANCH: SELECTED + NOT EDITING - applying highlight"
                        $beforeLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending selectColor='$($selectColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($selectColor)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending label='$label1'"
                        $sb.Append($label1)
                        $spaces = ' ' * (22 - $label1.Length)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending $($spaces.Length) spaces"
                        $sb.Append($spaces)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending reset='$($reset.Replace("`e", '<ESC>'))'"
                        $sb.Append($reset)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending highlightColor='$($highlightColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($highlightColor)
                        $afterLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 highlight complete: added=$($afterLen - $beforeLen) chars, sb.Length now=$afterLen"
                    } else {
                        # Normal rendering
                        if ($isSelected1) {
                            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 BRANCH: SELECTED + EDITING - NO highlight"
                        } else {
                            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 BRANCH: NOT SELECTED - normal render"
                        }
                        $beforeLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending mutedColor='$($mutedColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($mutedColor)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending label='$label1'"
                        $sb.Append($label1)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending reset='$($reset.Replace("`e", '<ESC>'))'"
                        ...
                        $sb.Append($spaces)
                        $afterLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 normal render complete: added=$($afterLen - $beforeLen) chars"
                    }

                    # Only render value if NOT in edit mode for this field
                    if (-not ($isSelected1 -and $this.ShowInlineEditor)) {
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 rendering value (not editing this field)"
                        $beforeLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending textColor='$($textColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($textColor)
                        $val1 = $(if ($field1.Value) { [string]$field1.Value } else { $mutedColor + "(empty)" + $reset })
                        if ($val1.Length -gt 18) { $val1 = $val1.Substring(0, 15) + "..." }
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending value='$val1'"
                        $sb.Append($val1)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 appending reset"
                        $sb.Append($reset)
                        $afterLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 value render complete: added=$($afterLen - $beforeLen) chars"
                    } else {
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 SKIPPING value render (editing this field, InlineEditor will render)"
                    }
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL1 render complete for fieldIdx=$fieldIndex"
                }
                ...
                $ts = Get-Date -Format 'HH:mm:ss.fff'
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   ROW $rowsRendered - editorActiveOnThisRow=$editorActiveOnThisRow (ShowInlineEditor=$($this.ShowInlineEditor) SelectedIndex=$($this.List.GetSelectedIndex()))"

                # Render COL2
                if ($field2) {
                    $isSelected2 = ($fieldIndex + 1 -eq $this.List.GetSelectedIndex())
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 render start: row=$rowsRendered fieldIdx=$($fieldIndex + 1) label=$($field2.Label) isSelected=$isSelected2 ShowInlineEditor=$($this.ShowInlineEditor)"

                    $moveToCode = $this.Header.BuildMoveTo($col2X, $rowY)
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 MoveTo: col2X=$col2X rowY=$rowY code='$($moveToCode.Replace("`e", '<ESC>'))'"
                    $sb.Append($reset)
                    $sb.Append($moveToCode)

                    $label2 = $field2.Label
                    if ($label2.Length -gt 20) { $label2 = $label2.Substring(0, 17) + "..." }
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 label truncated: '$label2' (original='$($field2.Label)')"

                    # Render label with selection highlight when selected (but not editing)
                    if ($isSelected2 -and -not $this.ShowInlineEditor) {
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 BRANCH: SELECTED + NOT EDITING - applying highlight"
                        $beforeLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending selectColor='$($selectColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($selectColor)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending label='$label2'"
                        $sb.Append($label2)
                        $spaces = ' ' * (22 - $label2.Length)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending $($spaces.Length) spaces"
                        $sb.Append($spaces)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending reset='$($reset.Replace("`e", '<ESC>'))'"
                        $sb.Append($reset)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending highlightColor='$($highlightColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($highlightColor)
                        $afterLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 highlight complete: added=$($afterLen - $beforeLen) chars"
                    } else {
                        if ($isSelected2) {
                            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 BRANCH: SELECTED + EDITING - NO highlight"
                        } else {
                            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 BRANCH: NOT SELECTED - normal render"
                        }
                        $beforeLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending mutedColor='$($mutedColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($mutedColor)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending label='$label2'"
                        $sb.Append($label2)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending reset='$($reset.Replace("`e", '<ESC>'))'"
                        ...
                        $sb.Append($spaces)
                        $afterLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 normal render complete: added=$($afterLen - $beforeLen) chars"
                    }

                    # Only render value if NOT in edit mode for this field
                    if (-not ($isSelected2 -and $this.ShowInlineEditor)) {
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 rendering value (not editing this field)"
                        $beforeLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending textColor='$($textColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($textColor)
                        $val2 = $(if ($field2.Value) { [string]$field2.Value } else { $mutedColor + "(empty)" + $reset })
                        if ($val2.Length -gt 18) { $val2 = $val2.Substring(0, 15) + "..." }
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending value='$val2'"
                        $sb.Append($val2)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 appending reset"
                        $sb.Append($reset)
                        $afterLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 value render complete: added=$($afterLen - $beforeLen) chars"
                    } else {
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 SKIPPING value render (editing this field)"
                    }
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL2 render complete for fieldIdx=$($fieldIndex + 1)"
                }

                # Render COL3
                if ($field3) {
                    $isSelected3 = ($fieldIndex + 2 -eq $this.List.GetSelectedIndex())
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 render start: row=$rowsRendered fieldIdx=$($fieldIndex + 2) label=$($field3.Label) isSelected=$isSelected3 ShowInlineEditor=$($this.ShowInlineEditor)"

                    $moveToCode = $this.Header.BuildMoveTo($col3X, $rowY)
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 MoveTo: col3X=$col3X rowY=$rowY code='$($moveToCode.Replace("`e", '<ESC>'))'"
                    $sb.Append($reset)
                    $sb.Append($moveToCode)

                    $label3 = $field3.Label
                    if ($label3.Length -gt 20) { $label3 = $label3.Substring(0, 17) + "..." }
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 label truncated: '$label3' (original='$($field3.Label)')"

                    # Render label with selection highlight when selected (but not editing)
                    if ($isSelected3 -and -not $this.ShowInlineEditor) {
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 BRANCH: SELECTED + NOT EDITING - applying highlight"
                        $beforeLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending selectColor='$($selectColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($selectColor)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending label='$label3'"
                        $sb.Append($label3)
                        $spaces = ' ' * (22 - $label3.Length)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending $($spaces.Length) spaces"
                        $sb.Append($spaces)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending reset='$($reset.Replace("`e", '<ESC>'))'"
                        $sb.Append($reset)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending highlightColor='$($highlightColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($highlightColor)
                        $afterLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 highlight complete: added=$($afterLen - $beforeLen) chars"
                    } else {
                        if ($isSelected3) {
                            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 BRANCH: SELECTED + EDITING - NO highlight"
                        } else {
                            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 BRANCH: NOT SELECTED - normal render"
                        }
                        $beforeLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending mutedColor='$($mutedColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($mutedColor)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending label='$label3'"
                        $sb.Append($label3)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending reset='$($reset.Replace("`e", '<ESC>'))'"
                        ...
                        $sb.Append($spaces)
                        $afterLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 normal render complete: added=$($afterLen - $beforeLen) chars"
                    }

                    # Only render value if NOT in edit mode for this field
                    if (-not ($isSelected3 -and $this.ShowInlineEditor)) {
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 rendering value (not editing this field)"
                        $beforeLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending textColor='$($textColor.Replace("`e", '<ESC>'))'"
                        $sb.Append($textColor)
                        $val3 = $(if ($field3.Value) { [string]$field3.Value } else { $mutedColor + "(empty)" + $reset })
                        if ($val3.Length -gt 18) { $val3 = $val3.Substring(0, 15) + "..." }
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending value='$val3'"
                        $sb.Append($val3)
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 appending reset"
                        $sb.Append($reset)
                        $afterLen = $sb.Length
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 value render complete: added=$($afterLen - $beforeLen) chars"
                    } else {
                        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 SKIPPING value render (editing this field)"
                    }
                    Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   COL3 render complete for fieldIdx=$($fieldIndex + 2)"
                }

                $fieldIndex += 3
                $rowY++
                $rowsRendered++
                Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') === ROW $($rowsRendered - 1) END: fieldIndex now=$fieldIndex rowY now=$rowY ==="
            }

            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Grid render complete: $rowsRendered rows rendered"
            ...
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   SKIPPING grid render - AllFields.Count is 0"
        }
        ...
        $result = $sb.ToString()
        $ts = Get-Date -Format 'HH:mm:ss.fff'
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts *** ProjectInfoScreenV2.RenderContent END ***"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Output length=$($result.Length) characters"
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   First 200 chars - $($result.Substring(0, [Math]::Min(200, $result.Length)).Replace("`e", '<ESC>').Replace("`n", '<LF>'))"

        # Dump the area around the selected field (CAA Name at position ~450-550)
        if ($result.Length -gt 600) {
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Chars 400-600 - $($result.Substring(400, [Math]::Min(200, $result.Length - 400)).Replace("`e", '<ESC>').Replace("`n", '<LF>'))"
        }
        if ($result.Length -gt 800) {
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts   Chars 600-800 - $($result.Substring(600, [Math]::Min(200, $result.Length - 600)).Replace("`e", '<ESC>').Replace("`n", '<LF>'))"
        }

        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$ts ========================================="
        return $result
    }

    # Override EditItem to edit a field using InlineEditor (TaskListScreen pattern)
    [void] EditItem($field) {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** ProjectInfoScreenV2.EditItem CALLED ***"
        if ($global:PmcTuiLogFile) {
        ...
        if ($null -eq $field) {
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** ProjectInfoScreenV2.EditItem field is NULL, returning ***"
            return
        }
        ...
        $this.InlineEditor.OnConfirmed = {
            param($values)
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** OnConfirmed CALLBACK - fieldName=$fieldName value='$($values[$fieldName])' ***"
            ...
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Editor closed, ShowInlineEditor=$($self.ShowInlineEditor)"
            ...
            Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff')   Selection restored to index $savedIndex"
    ...
    [void] OnItemActivated($item) {
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** ProjectInfoScreenV2.OnItemActivated CALLED ***"
        if ($global:PmcTuiLogFile) {
        ...
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** ProjectInfoScreenV2.OnItemActivated about to call EditItem ***"
        # Edit the selected field
        $this.EditItem($item)
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') *** ProjectInfoScreenV2.OnItemActivated EditItem returned ***"
    }
