...
class InlineEditor : PmcWidget {
    ...
    hidden [void] _CreateFieldWidget([hashtable]$fieldDef) {
        ...
        Add-Content -Path "$($env:TEMP)/pmc-flow-debug.log" -Value "$(Get-Date -Format 'HH:mm:ss.fff') [_CreateFieldWidget] Creating widget for field=$fieldName type=$fieldType LayoutMode=$($this.LayoutMode) value=$(if ($null -ne $value) { $value } else { '(null)' })"
